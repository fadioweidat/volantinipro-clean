-- DEPLOY-PLAN-3 — Territory/civici delta, production-safe renumbering.
--
-- Renumbered from supabase/migrations/20260805000013_address_points_radius_summary.sql
-- (content unchanged). Verified via direct read-only query on 2026-08-06:
--   - public.address_points EXISTS on remote with 337,049 real rows and the
--     exact column set this file expects (source, comune, codice_comune,
--     via, numero_civico, lat, lng, confidence, geom).
--   - public.get_address_points_radius_summary(...) CONFIRMED ABSENT.
--   - The partial GIST index address_points_osm_geography_idx CONFIRMED ABSENT.
-- This migration is purely additive: it adds one index and one read-only
-- SECURITY INVOKER function. It does not touch, reinsert, or overwrite any
-- of the 337,049 existing address_points rows.
