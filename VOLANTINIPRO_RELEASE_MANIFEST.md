...
> **Aggiornamento — RC2-BROWSER-E2E-1 + RC2-FINALIZE-1 (2026-08-06/07).** Il flusso Admin→Driver è stato eseguito **realmente in browser** (non solo via SQL/RPC) contro uno stack Supabase locale isolato ma schema-compatibile con il remoto reale (Postgres+GoTrue+PostgREST+Kong+Mailpit, catena `150001`-`150009` applicata). Durante il test sono stati trovati e corretti, con autorizzazione esplicita per ciascuno, due bug reali nel frontend: (1) `useGpsTracking.js` non esponeva `path`/`distanceKm`/`speed`/`heading`/`assignmentStatus`/`assignmentError` richiesti da `DriverAssignmentPage.jsx` (crash immediato della pagina Driver); (2) `DriverAssignmentPage.jsx` bloccava l'intera pagina su assegnazione revocata anche con una sessione GPS già attiva/in pausa, impedendo di raggiungere Termina — corretto per permettere l'accesso solo-Termina quando esiste una sessione live, lasciando il backend (`gps_transition_session`) come unica source of truth. Dettaglio completo: [ADMIN_DRIVER_LINK_FLOW_BROWSER_E2E_REPORT.md](ADMIN_DRIVER_LINK_FLOW_BROWSER_E2E_REPORT.md). Verdetto: **PASSED**. 2 commit locali aggiuntivi su questo stesso worktree (`122a0d6` fix, `3f9c8e1` report). Pipeline rieseguita dopo `npm ci`: 280/280, build OK, `git diff --check` pulito, `git status --short` vuoto. Tag annotato locale creato: `volantinipro-rc2-2026-08-07`, puntato sul commit finale effettivo di questo worktree (vedi Fase 6 di RC2-FINALIZE-1). Nessuna scrittura sul database remoto, nessun push, nessun deploy.

> **Aggiornamento — RC2-FREEZE-AND-RESTORE-1 (2026-08-07).** Nuovo backup RC2 reale eseguito (indipendente dal backup RC1 del giorno precedente, che non copriva la migrazione `150009`) e integralmente verificato tramite restore su clone Postgres isolato: schema pubblico identico al remoto (66 tabelle/802 funzioni/26 trigger/91 policy/189 indici), 12/12 tabelle critiche con conteggio dati esatto, `geo_nil_milano` confermato 88/88, RPC NIL funzionante con dati reali. **Durante la verifica del comando `--dry-run` è stato trovato un bug critico**: un blocco di commento nel file `20260806150009_admin_driver_assignment_flow.sql`, posizionato dopo `commit;` invece che prima di `begin;`, impediva al runner di riconoscere e rimuovere il `commit;` letterale del file — in modalità `--dry-run` questo committava realmente l'intera catena accumulata, vanificando la garanzia di non-distruttività (dimostrato: dopo un primo dry-run su clone, tutte e 8 le migrazioni DDL rilevate dal runner risultavano persistite — la cartella contiene 9 file, di cui 8 migrazioni DDL e 1 script di sola verifica, `150004`, escluso di proposito dal runner e non applicabile in senso stretto). **Nessun impatto su `--apply`** (già commit-per-file per design) e **nessun danno reale** (mai eseguito contro il remoto). Fix minimo autorizzato applicato — solo riposizionamento del blocco di commento, zero modifiche alla logica SQL — e riverificato da zero: dry-run rieseguito su clone pulito, confermato con query dirette **0 oggetti persistiti, 0 righe ledger**. Applicata poi la catena completa (`--apply`, 8/8 successo — le 8 migrazioni DDL della catena; `150004`, script di sola verifica, escluso di proposito dal runner ed eseguito manualmente a parte con esito positivo), riapplicata una seconda volta (8/8 SKIPPED, nessun duplicato), testate funzionalmente tutte le RPC di `150009` (5 Admin + matrice sicurezza Driver a 7 vie + ciclo revoca completo con audit log coerente), verificata la regressione completa (NIL/OMI/POI/GTFS/AI/GPS/coverage/RLS tutti invariati o correttamente popolati), eseguito un test di rollback controllato (fallimento simulato → rollback per-file → ledger coerente → file successivi bloccati → restart-dopo-fix corretto). Dettaglio completo: [VOLANTINIPRO_RC2_FREEZE_REPORT.md](VOLANTINIPRO_RC2_FREEZE_REPORT.md), [VOLANTINIPRO_RC2_BACKUP_MANIFEST.md](VOLANTINIPRO_RC2_BACKUP_MANIFEST.md), [VOLANTINIPRO_RC2_RESTORE_TEST_REPORT.md](VOLANTINIPRO_RC2_RESTORE_TEST_REPORT.md).

> **Aggiornamento — RC2-FREEZE-FINALIZE-2 (2026-08-07).** Verificato con rigore, su richiesta esplicita, il presunto scarto "9 migrazioni dichiarate vs. 8 applicate": **non è un file mancante**. `supabase/migrations_production_safe/` contiene 9 file reali (SHA-256 di ognuno calcolato e verificato); di questi, 8 sono migrazioni DDL vere e proprie (rilevate/applicate/ledgerate dal runner) e 1 (`20260806150004_gps_post_migration_verify.sql`) è dichiarato nel proprio header come script di sola verifica ("NOT a schema migration... run manually... changes nothing") — il runner lo esclude di proposito, comportamento confermato corretto e non modificato. `150004` eseguito manualmente sul clone: tutte le sue verifiche passate. Il fix del bug dry-run (commit isolato `d20c861`, confinato esclusivamente a `20260806150009`, zero modifiche a `scripts/deploy-production-migrations.mjs`) è stato committato, seguito da un commit documentale separato con i 5 report/manifest aggiornati. Pipeline rieseguita dopo entrambi i commit: 280/280, build OK, worktree completamente pulito. **Verdetto: RC2 FREEZE AND RESTORE READY — GO.** Nuovo tag annotato locale: `volantinipro-rc2-2026-08-07b`, puntato sul commit finale. Nessuna migrazione applicata al remoto, nessun push, nessun deploy.
