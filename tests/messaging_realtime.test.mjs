...
test("DriverAssignmentPage.jsx: polling adattivo e network recovery presenti", () => {
  ...
  assert.ok(driverPageSrc.includes("window.addEventListener('focus', onFocus)"), "deve ricaricare immediatamente quando la finestra torna in focus");
  ...
  assert.ok(driverPageSrc.includes("document.visibilityState === 'visible'"), "il fallback periodico deve interrogare solo quando la pagina e' visibile");
  ...
  assert.ok(driverPageSrc.includes("window.removeEventListener('focus', onFocus)"), "cleanup listener focus");
  assert.ok(driverPageSrc.includes("document.removeEventListener('visibilitychange', onVisibility)"), "cleanup listener visibilitychange");
});
