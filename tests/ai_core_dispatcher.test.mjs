...
test('ai-core: contextType validi non implementati rispondono 501 e i context implementati hanno rami dedicati', () => {
  ...
  assert.match(contextTypesSource, /"step1"[\s\S]*"step2"[\s\S]*"step3"[\s\S]*"step4"[\s\S]*"customer_dashboard"[\s\S]*"admin_dashboard"[\s\S]*"driver_assignment"[\s\S]*"supplier_dashboard"[\s\S]*"territorial_report"[\s\S]*"campaign_report"/);
  assert.match(contextTypesSource, /IMPLEMENTED_CONTEXT_TYPES\s*=\s*Object\.freeze\(\[\s*"step1"[\s\S]*"driver_assignment"[\s\S]*"supplier_dashboard"[\s\S]*"territorial_report"[\s\S]*\]\)/);
});
