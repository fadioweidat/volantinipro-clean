...
async function setupFixtures() {
  ...
  dbExec(`
    ...
    values
      (
        ...
        'active',
        true,
        'gps4m_assignment_fixture',
        ${sqlJson({ gps4m_fixture: true, runId, owner: 'client_a' })}
      ),
      (
        ${sqlUuid(campaignBId)},
        ${sqlUuid(clientB.id)},
        ...
        true,
        'gps4m_assignment_fixture',
        ${sqlJson({ gps4m_fixture: true, runId, owner: 'client_b' })}
      );
  ...
  return {
    ...
    campaigns: {
      campaignA: { id: campaignAId, user_id: clientA.id, customer_id: clienteAId },
      campaignB: { id: campaignBId, user_id: clientB.id, customer_id: clienteBId },
    },
