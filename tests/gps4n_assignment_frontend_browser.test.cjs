...
test('UI operatore minima espone stato assegnazione e blocca start', () => {
  assert.match(trackingPage, /const canStart = tracking\.isAssignmentReady/);
  assert.match(trackingPage, /disabled=\{!canStart\}/);
  assert.match(trackingPage, /tracking\.assignmentStatus === 'loading'/);
  assert.match(trackingPage, /tracking\.assignmentError/);
  assert.match(trackingPage, /label="Assignment"/);
  assert.match(trackingPage, /label="ID assignment"/);
});
