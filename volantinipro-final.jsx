...
export const F = { serif: "'DM Serif Display',Georgia,serif", sans: "'DM Sans',sans-serif" };
...
export function Navbar({ onNav, page }) {
...
const navLinks = ["Funzionalit", "Prezzi", "Risorse", "Chi siamo"];
...
// Section
export function StepperBar({ current, onGo }) {
...
const CAP_LOMBARDIA = [
  ...
  {postal_code:"20130",municipality_name:"Milano (Citt Studi)"},
  ...
  {postal_code:"20053",municipality_name:"Muggi"},
...
export function Step2({ data, setData, onNext, onBack }) {
  const isMobile = useIsMobile();
...
const radiusInsightRows = zonesInRadius.map(z => ({
  ...
  const mainTargetLabel = isResidentialStep2 ? "Famiglie stim." : isMovementStep2 ? "POI rilevanti" : "Attivit tot.";
  ...
  const advancedScoreRows = [
    ...baseScoreRows,
    ...(isResidentialStep2 ? [
      { l: "Densit media", v: serviceKpis.area && serviceKpis.population ? Math.round(serviceKpis.population / Number(serviceKpis.area)) : null, c: C.blue },
          ...
          <div style={{
            ...
            {isBusinessStep2 && city && (
              <div style={{ position: "absolute", top: 10, left: 10, background: "rgba(8,15,30,.88)", borderRadius: 8, padding: "8px 10px", border: "1px solid rgba(255,255,255,.08)", maxWidth: 220 }}>
                ...
                {[
                  { c: targetBusinessMeta.color, l: "attivit target / categoria" },
              ...
              <div style={{ padding: "12px 14px", borderBottom: "1px solid rgba(255,255,255,.06)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ padding: "2px 7px", borderRadius: 100, background: `${col}18`, fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: col }}>Modalit CAP</span>
                  </div>
            ...
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, border: "1px solid rgba(255,255,255,.07)", overflow: "hidden" }}>
              ...
              <div style={{ padding: "10px 14px", background: "rgba(255,255,255,.02)", borderBottom: "1px solid rgba(255,255,255,.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.45)", lineHeight: 1.4, maxWidth: 400 }}>
                  {allocationMode === "auto"
                    ? "Il sistema distribuisce automaticamente i volantini partendo dalle zone pi vicine e pi coerenti."
                    : `Modalit manuale: scegli tu quanti volantini assegnare a ogni ${territorySingularLabel.toLowerCase()}.`
                  }
                </div>
...
const isManual = allocationMode === "manual";
                  ...
                  const coverageLabel = coverageState === "none" ? "In raggio, non coperto" : coverageState === "full" ? "Copertura Totale" : "Copertura Parziale";
                              ...
                              <div>
                                {assignedFlyers === 0 ? (
                                  ...
                                  <>
                                    ...
                                    <div style={{ fontFamily: F.sans, fontSize: 8, color: "rgba(255,255,255,.3)" }}>di {requiredFlyers.toLocaleString("it-IT")} necessari</div>
                                  </>
                                )}
                              </div>
                            )
                    ...
                    isPartial ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        <div style={{ padding: "10px 14px", borderRadius: 10, background: "rgba(232,87,26,.08)", border: "1px solid rgba(232,87,26,.28)", fontFamily: F.sans, fontSize: 11, color: C.orange, lineHeight: 1.5 }}>
                          {isResidentialStep2 ? `Copertura parziale: hai ${flyerQuantityFromStep1.toLocaleString("it-IT")} vol., servono ${requiredFlyers.toLocaleString("it-IT")}. Puoi continuare con copertura parziale.` : "Copertura parziale: quantit insufficiente. Puoi continuare con copertura parziale."}
                        </div>
                      ...
                      <div style={{ padding: "10px 14px", background: "rgba(46,204,138,.08)", border: "1px solid rgba(46,204,138,.2)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ fontFamily: F.sans, fontSize: 12, fontWeight: 700, color: C.green }}>quantit sufficiente per la copertura totale.</div>
                        {remainingFlyers > 0 && (
                      ...
                      {isInvalid ? (
                        <div style={{ background: "rgba(248,113,113,.1)", border: "1px solid rgba(248,113,113,.3)", borderRadius: 10, padding: "10px 14px" }}>
                          ...
                          <div style={{ fontFamily: F.sans, fontSize: 11, color: "rgba(255,255,255,.7)" }}>
                            ...
                            Riduci una zona o aumenta la quantit.
                          </div>
                        </div>
                      ) : (
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        {isInvalid && (
                          <button onClick={() => setData(d => ({...d, qty: totalAssigned, flyerQuantity: totalAssigned }))}
                            ...
                            Aumenta quantit a {totalAssigned.toLocaleString("it-IT")}
                          </button>
                        )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          ...
          {searchMode === "cap" && selectedCaps.length === 0 && (
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, padding: "18px", border: `1px solid ${col}22`, textAlign: "center" }}>
              ...
              <div style={{ fontFamily: F.serif, fontSize: 16, color: C.white, marginBottom: 6 }}>Modalit CAP attiva</div>
          ...
          <div style={{ background: `${col}10`, borderRadius: 12, padding: "14px 16px", border: `1px solid ${col}28` }}>
            <div style={{ fontFamily: F.sans, fontSize: 9, fontWeight: 700, color: col, letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 6 }}>
              {searchMode === "cap" ? "Summary  Modalit CAP" : isResidentialStep2 ? "Summary  area residenziale" : isBusinessStep2 ? "Summary  area business" : isMovementStep2 ? "Summary  area operativa" : "Totale selezione"}
            </div>
            ...
            {selZones.length > 0 && (
              <div style={{ padding: "8px 10px", borderRadius: 8, background: coverageStatus === "sufficient" ? "rgba(46,204,138,.08)" : "rgba(248,113,113,.08)", border: `1px solid ${coverageStatus === "sufficient" ? "rgba(46,204,138,.22)" : "rgba(248,113,113,.24)"}` }}>
                <div style={{ fontFamily: F.sans, fontSize: 11, fontWeight: 700, color: coverageStatus === "sufficient" ? C.green : C.red, marginBottom: 2 }}>
                  {coverageStatus === "sufficient" ? "quantit sufficiente" : "Volantini insufficienti"}
                </div>
                <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.52)", lineHeight: 1.45 }}>
                  {coverageStatus === "sufficient"
                    ? `quantit sufficiente per coprire le zone selezionate. Avanzano ${flyerSurplus.toLocaleString("it-IT")} volantini.`
              ...
              {selZones.length === 0 && !isMovementStep2 && !isBusinessStep2 ? (
                ...
                <div style={{ padding: "8px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                  {(isResidentialStep2 ? [
                    ...
                    { l: "Giorni operativi", v: aiAgg?.operDays, c: C.yellow },
                    { l: "Cassette stimate", v: aiAgg?.mailboxes, format: "number", c: col },
                  ] : isMovementStep2 ? [
                    ...
                    { l: "Attivit rilevate", v: serviceKpis.businesses, format: "number", c: col },
                    ...
                    { l: "Waypoint GPS", v: gisLoading ? gisKpi(null, 44) : serviceKpis.gpsWaypoints, format: "number", c: C.yellow },
                    { l: "Attivit target", v: gisLoading ? gisKpi(null, 58) : serviceKpis.targetBusinesses, format: "number", c: C.green },
                  ] : [
                      ...
                      <div key={i} style={{ padding: "6px 8px", borderRadius: 8, background: "rgba(232,87,26,.06)", border: "1px solid rgba(232,87,26,.14)", marginBottom: 4 }}>
                        ...
                        <div style={{ fontFamily: F.sans, fontSize: 13, fontWeight: 800, color: C.orange }}>
                          {v.min_value != null && v.max_value != null ? (
                            <>
                              {v.min_value.toLocaleString("it-IT")}{"  "}{v.max_value.toLocaleString("it-IT")}
            ...
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, border: "1px solid rgba(255,255,255,.07)", overflow: "hidden" }}>
              ...
              <div style={{ padding: "8px 10px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5 }}>
                {[
                  ...
                  { l: "Densit", v: serviceKpis.area && serviceKpis.pop ? `${formatNumber(Math.round(serviceKpis.pop / Number(serviceKpis.area)))} ab/km` : null },
...
const unavailable = v === null || v === undefined || v === "" || String(v).includes("undefined");
                    ...
                    <div style={{ padding: "9px 10px", borderRadius: 9, background: "rgba(232,87,26,.08)", border: "1px solid rgba(232,87,26,.2)", fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.58)", lineHeight: 1.45 }}>
                      Lettura residenziale: priorit a comuni con alta concentrazione famiglie, buona copertura cassette e quantit coerente con il fabbisogno stimato.
                    </div>
                  </div>
                ) : isResidentialStep2 ? (
                  <div style={{ padding: "10px", display: "grid", gap: 10 }}>
                    <div style={{ borderRadius: 10, background: "rgba(255,255,255,.035)", border: "1px solid rgba(255,255,255,.06)", overflow: "hidden" }}>
                      ...
                      <div style={{ padding: "7px 10px" }}>
                        {[
                          ...
                          { l: "Densit media", v: serviceKpis.area && (serviceKpis.population || aiAgg.pop) ? `${formatNumber(Math.round((serviceKpis.population || aiAgg.pop) / serviceKpis.area))} ab./km` : null, c: C.white },
                    ...
                    <div style={{ borderRadius: 10, background: "rgba(255,255,255,.035)", border: "1px solid rgba(255,255,255,.06)", overflow: "hidden" }}>
                      ...
                      <div style={{ padding: "7px 10px" }}>
                        {demoLoading ? (
                          ...
                          { l: "Et 0-14",  v: demoData?.age_0_14_pct  != null ? `${demoData.age_0_14_pct.toFixed(1)}%`  : null, c: "#93C5FD" },
                          { l: "Et 15-34", v: demoData?.age_15_34_pct != null ? `${demoData.age_15_34_pct.toFixed(1)}%` : null, c: "#34D399" },
                          { l: "Et 35-64", v: demoData?.age_35_64_pct != null ? `${demoData.age_35_64_pct.toFixed(1)}%` : null, c: "#FBBF24" },
                          { l: "Et 65+",   v: demoData?.age_65_plus_pct != null ? `${demoData.age_65_plus_pct.toFixed(1)}%` : null, c: "#F87171" },
                        ].map(({ l, v, c }) => (
                          <div key={l} style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "5px 0", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                            <span style={{ fontFamily: F.sans, fontSize: 9, color: "rgba(255,255,255,.38)" }}>{l}</span>
                            <span style={{ fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: v == null ? "rgba(255,255,255,.28)" : c, textAlign: "right", maxWidth: 170 }}><MetricValue value={v} /></span>
...
const unavailable = v === null || v === undefined || v === "" || String(v).includes("undefined");
                    ...
                    <div style={{ padding: "9px 10px", borderRadius: 9, background: "rgba(96,165,250,.08)", border: "1px solid rgba(96,165,250,.2)", fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.58)", lineHeight: 1.45 }}>
                      Lettura operativa: priorit a punti con flusso alto, anchor trasporto/scuola-evento e fasce orarie coerenti. Le metriche demografiche restano secondarie.
                    </div>
                  </div>
                ) : isBusinessStep2 ? (
                  <div style={{ padding: "10px", display: "grid", gap: 10 }}>
...
const unavailable = v === null || v === undefined || v === "" || String(v).includes("undefined");
                    ...
                    <div style={{ padding: "9px 10px", borderRadius: 9, background: `${targetBusinessMeta.color}10`, border: `1px solid ${targetBusinessMeta.color}22`, fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.58)", lineHeight: 1.45 }}>
                      Lettura operativa: priorit alle zone con pi attivit target, cluster forti e densit commerciale alta. I competitor sono contesto decisionale, non blocco automatico.
                    </div>
                  </div>
            ...
            {showAdv && (
              <div style={{ padding: "10px" }}>
                {!firstZ
                  ...
                  : <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                    {advancedScoreRows.map((s, i) => <ScoreGauge key={i} label={s.l} value={s.v} color={s.c} />)}
                  </div>
                }
              </div>
            )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: "auto" }}>
            ...
            {!step2ZonesReady && (
              <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.38)", textAlign: "center", lineHeight: 1.5, padding: "0 4px" }}>
                Assicurati che tutte le zone abbiano un'area geografica e una quantit di volantini valida.
              </div>
            )}
...
export function Step3({ data, setData, onNext, onBack }) {
  const isMobile = useIsMobile();
...
export function Step4({ data, setData, onBack, onHome, onCampaignSaved }) {
  const isMobile = useIsMobile();
...
const d2dScores = [
    { l: "Family Index", v: kpis.familyIndex ?? avgFIdx, c: C.orange, d: "Qualit residenziale dell'area", src: "Analisi interna" },
    ...
    { l: "Confidence Score", v: kpis.confidenceScore ?? (selZ.length ? Math.round(selZ.reduce((a, z) => a + z.confD2D, 0) / selZ.length) : 0), c: C.purple, d: "Affidabilit della stima", src: "Analisi interna" },
  ];
...
const serviceSummaryConfig = {
    d2d: {
      ...
      admin: [
        ...
        { l: "Et 65+", v: (() => { const vs = selZ.map(z => z.eta65).filter(n => n != null); return vs.length ? `${Math.round(vs.reduce((a,n) => a+n, 0)/vs.length*10)/10}%` : null; })() },
    ...
    h2h: {
      ...
      fields: [
        ...
        { l: "Densit passaggio", v: kpis.passageDensity != null ? `${kpis.passageDensity}/100` : null, src: "Analisi interna", c: C.orange },
      ...
      scores: [
        ...
        { l: "Confidence Score", v: kpis.confidenceScore, c: C.orange, d: "Affidabilit della stima" },
      ],
      admin: [
        { l: "Hotspot principale", v: kpis.strongestHotspot },
        { l: "Attivit vicine", v: kpis.nearbyBiz ? formatNumber(kpis.nearbyBiz) : null },
    ...
    b2b: {
      ...
      fields: [
        { l: "Attivit rilevate", v: formatNumber(kpis.businesses), src: "Google Places", c: C.purple },
        ...
        { l: "Attivit target", v: formatNumber(kpis.targetBusinesses), src: "Google Places", c: C.green },
      ],
      scores: [
      ...
      admin: [
        ...
        { l: "Zona pi forte", v: kpis.strongestZone },
  ...
  if (svcType === "d2d") {
    serviceSummaryConfig.admin = [
      ...
      { l: "Densit media", v: d2dAvgDensity ? `${formatNumber(d2dAvgDensity)} ab./km` : null, src: "ISTAT" },
...
function handleDownloadPdf() {
    ...
    try {
      ...
      setPdfError("Non  stato possibile aprire il preventivo. Controlla che i popup siano abilitati.");
    } finally {
      setPdfBusy(false);
    }
  }
          ...
          <div style={{...box(), padding: "18px" }}>
            ...
            {isQuick ? (
              <div style={{ padding: "14px", borderRadius: 10, background: "rgba(255,255,255,.025)", border: "1px solid rgba(255,255,255,.06)", fontFamily: F.sans, fontSize: 12, color: "rgba(255,255,255,.42)", lineHeight: 1.55 }}>
                Preventivo rapido: il dettaglio completo di KPI, comuni e profilo demografico sar disponibile dopo l'analisi zona completa.
              </div>
            ) : (
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(145px,1fr))", gap: 8 }}>
                    {nonEmpty([
                      ...
                      { l: "Modalit selezione", v: data.allocationMode === "manual" ? "Manuale" : "Auto", src: "Step 2", c: col },
                    ]).map(fieldCard)}
            ...
            {selZ.length > 0 && svcType !== "d2d" && (
              <div style={{ marginTop: 8, padding: "8px 10px", borderRadius: 8, background: "rgba(96,165,250,.07)", border: "1px solid rgba(96,165,250,.18)", fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.55)", lineHeight: 1.45 }}>
                {svcType === "h2h"
                  ...
                  : "Business Distribution usa quantit mirate: il conteggio si basa su attivit target e cluster commerciali, non su copertura residenziale completa."}
              </div>
            )}
          </div>
...
const remaining = campaign.qty - required;
  ...
  if (campaign.svc === "h2h") {
    return {
      ...
      rows: [
        ...
        { l: "Densit passaggio", v: `${Math.round(zones.reduce((a, z) => a + z.commDens, 0) / zones.length)}/100` },
  ...
  return {
    zones,
    rows: [
      { l: "Attivit rilevate", v: zones.reduce((a, z) => a + z.bizTotal, 0).toLocaleString("it-IT") },
      ...
      { l: "Attivit target", v: zones.reduce((a, z) => a + z.targetBiz, 0).toLocaleString("it-IT") },
...
const paymentStatus = c.stato_pagamento || (c.status === "done" ? "pagato" : "in_attesa");
          ...
          <div style={{...box(), overflow: "hidden" }}>
            <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,.07)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,.35)", letterSpacing: ".1em", textTransform: "uppercase" }}>Mappa operativa  campagne e compatibilit</div>
                ...
                return (
                  <div key={i} style={{ padding: "11px 14px", borderBottom: "1px solid rgba(255,255,255,.04)", transition: "background.12s" }}
                    ...
                    <div style={{ padding: "7px 8px", borderRadius: 8, background: "rgba(46,204,138,.08)", border: "1px solid rgba(46,204,138,.16)", marginBottom: 7 }}>
                      <div style={{ fontFamily: F.sans, fontSize: 10, color: C.green, fontWeight: 800 }}>Compatibilit operativa</div>
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                      ...
                      <button onClick={() => { setFilterOp("compatible"); setFilterStatus("all"); setFilterSvc("all"); }}
                        style={{ padding: "4px 9px", borderRadius: 7, border: `1px solid ${C.orange}25`, background: `${C.orange}10`, color: C.orange, fontFamily: F.sans, fontSize: 10, fontWeight: 700, cursor: "pointer" }}>Apri compatibilit</button>
...
const { icon, label, col } = SVC_BADGE[svc];
          ...
          <div style={{...box(), padding: "14px" }}>
            ...
            {[
              ...
              { icon: "", l: "Ricalcola compatibilit", disabled: true, hint: "Non ancora disponibile", col: C.green },
          ...
          <div style={{...box(), padding: "14px" }}>
            ...
            {[
              ...
              { icon: "", msg: "Compatibilit zona trovata - Paolo Greco ? C007", t: "08:52" },
...
export function LoginPage({ onNav }) {
...
export function CampaignDashboardPage({ onNav, campaignId }) {
...
export function PagamentoBonificoPage({ onNav, campaignId }) {
...
export function LegalPage({ type, onNav }) {
  const config = {
    privacy: {
      eyebrow: "Privacy",
...
function Step1ZoneCountSelector({ setData }) {
  ...
  return (
    <div style={{ marginTop: 16, marginBottom: 48 }}>
      <button
        type="button"
        onClick={() => ensureZones("multi")}
        onMouseEnter={event => { event.currentTarget.style.color = C.orangeHover || "#D14A14"; }}
        onMouseLeave={event => { event.currentTarget.style.color = C.orange; }}
        style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 12px", border: "none", background: "transparent", color: C.orange, cursor: "pointer", fontFamily: F.sans, fontSize: 14, fontWeight: 600, transition: "color .16s" }}
      >
        <span aria-hidden="true" style={{ width: 24, height: 24, borderRadius: "50%", border: `1.5px solid ${C.orange}`, display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 16, lineHeight: 1, flexShrink: 0 }}>+</span>
        Aggiungi un'altra zona / comune
      </button>
      <p style={{ fontFamily: F.sans, fontSize: 12, lineHeight: 1.5, color: "rgba(255,255,255,.42)", maxWidth: 430, margin: "8px 0 0" }}>
