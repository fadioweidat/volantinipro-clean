...
const go = (target) => { setMenuOpen(false); onNav(target); };
  return (
    <nav style={{ position: navPosition, top: 0, left: 0, right: 0, zIndex: 200, background: dark || sc ? "rgba(10,18,34,.97)" : "transparent", borderBottom: (dark || sc) ? "1px solid rgba(255,255,255,.07)" : "none", transition: "background.35s", backdropFilter: "blur(12px)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: isMobile ? "0 16px" : "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={() => go("home")}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: C.orange, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="17" height="17" viewBox="0 0 20 20" fill="none"><path d="M3 17L10 3L17 17H3Z" fill="white" opacity=".9" /><circle cx="10" cy="12" r="2.4" fill="white" /></svg>
          </div>
          <span style={{ fontFamily: F.serif, fontSize: 19, color: C.white }}>Volantini<span style={{ color: C.orange }}>Pro</span></span>
        </div>
        ...
        {!isMobile && <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <button onClick={() => go("admin")} style={{ padding: "9px 14px", borderRadius: 9, border: "1px solid rgba(232,87,26,.3)", background: "rgba(232,87,26,.07)", color: C.orange, fontFamily: F.sans, fontSize: 13, fontWeight: 800, cursor: "pointer" }}>Area Admin</button>
          ...
          return (
            <div key={s} style={{ display: "flex", alignItems: "center", flex: 1 }}>
              <div onClick={() => done && onGo(ids[i])} style={{ display: "flex", alignItems: "center", gap: 9, cursor: done ? "pointer" : "default", opacity: done || active ? 1 :.36 }}>
                ...
                <span style={{ fontFamily: F.sans, fontSize: 13, fontWeight: active ? 700 : 500, color: active ? C.white : "rgba(255,255,255,.52)", whiteSpace: "nowrap" }}>{s}</span>
              </div>
      ...
      <div style={{ padding: 18, display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {zones.map((z, i) => (
          <div key={z.name} onClick={() => setActive(i)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 9, cursor: "pointer", background: active === i ? "rgba(232,87,26,.1)" : "rgba(255,255,255,.03)", border: `1px solid ${active === i ? "rgba(232,87,26,.25)" : "rgba(255,255,255,.05)"}`, transition: "all.2s" }}>
            ...
            <span style={{ fontFamily: "monospace", fontSize: 11, color: "rgba(255,255,255,.45)" }}>{z.families.toLocaleString("it-IT")} famiglie</span>
  ...
  return (
    <div style={{ padding: "8px", borderRadius: 8, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.06)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ fontFamily: F.serif, fontSize: 12, color: color, fontWeight: 700 }}>{value}</div>
      </div>
    </div>
  );
}
...
const yr="div";
const Ov=[{value:"retail",label:"Retail / Negozio"},{value:"food",label:"Ristorazione / Food"},{value:"servizi",label:"Servizi alla persona"},{value:"salute",label:"Salute / Benessere"},{value:"immobiliare",label:"Immobiliare"},{value:"gdo",label:"GDO / Supermercati"},{value:"altro",label:"Altro"}];
...
const Iv=[{value:50,label:"Circa 50 attivita"},{value:100,label:"Circa 100 attivita"},{value:200,label:"Circa 200 attivita"},{value:500,label:"Circa 500 attivita"}];
...
const Kp=[{id:"gps_pro",icon:"",label:"Tracking GPS Live",price:"49€ flat",desc:"Accesso mappa GPS in tempo reale durante la campagna",col:C.blue},{id:"proof_foto",icon:"",label:"Proof Fotografico",price:"29€ flat",desc:"Report fotografico (20-30 foto) dei volantini consegnati",col:C.purple},{id:"report_plus",icon:"",label:"Report Analytics",price:"39€ flat",desc:"Analisi post-campagna dettagliata su conversioni stimate",col:C.green}];
...
function Step1({data:n,setData:i,onNext:r}){const[showSmartPairingModal,setShowSmartPairingModal]=React.useState(false);const l=useIsMobile(),u=[{id:"d2d",name:"Door to Door",code:"D2D",desc:"Distribuzione in cassette postali, condomini, palazzi, villette e zone residenziali.",features:["Famiglie stimate","Popolazione stimata","Copertura zona","Volantini consigliati","Comuni nel raggio"],c:C.orange},{id:"h2h",name:"Hand to Hand",code:"H2H",desc:"Distribuzione manuale in punti ad alto passaggio.",features:["POI rilevanti","Fermate metro/bus/treno","Scuole, universit, eventi","Flusso potenziale","Smart Pairing"],c:C.blue,popular:!0},{id:"b2b",name:"Business Distribution",code:"B2B",desc:"Distribuzione mirata ad attivita commerciali, uffici e zone business.",features:["Attivita rilevate","Categorie commerciali","Competitor vicini","Densita commerciale","Cluster zona"],c:C.purple}],h={single:1,monthly3:3,monthly6:6,monthly12:12},f={single:0,monthly3:5,monthly6:10,monthly12:15},m=n.type==="h2h",y=n.type==="b2b"||n.type==="business-distribution",x=!!(n.type&&n.qty>0&&n.flyerFormat&&n.hasFlyers&&n.urgency&&n.subscription&&(!m||n.promoterCount&&n.timeSlot&&n.serviceDurationHours&&(n.distributionLocation||n.distributionPointType))&&(!y||n.targetBusinessType&&n.businessCategory&&n.targetBusinessCount&&(n.businessZone||n.cityName))),w=!!(n.startDate&&n.endDate&&n.endDate<n.startDate),j=A=>{if(!A)return"";
...
const[,B,P,J]=F,V=new Date(Number(J),Number(P)-1,Number(B));return V.getFullYear()!==Number(J)||V.getMonth()!==Number(P)-1||V.getDate()!==Number(B)?"":`${J}-${P}-${B}`},R=(n.campaignsPerMonth||1)*(h[n.subscription]||1),D=A=>i(F=>{const B={...F,...A},P=h[B.subscription]||1,J=f[B.subscription]||0,V=B.subscription==="single"?1:B.campaignsPerMonth||1;return{...B,campaignsPerMonth:V,selectedService:B.type,businessSector:B.activityType,flyerQuantity:B.qty,campaignPeriodStart:B.startDate,campaignPeriodEnd:B.endDate,alreadyPrinted:B.hasFlyers==="yes",printServices:(B.extraServices||[]).filter(H=>["stampa","grafica"].includes(H)),paperWeight:B.printGramm,printSides:B.printSide,colorMode:B.printColor,campaignPlan:B.subscription,totalCampaigns:V*P,planDiscount:J,promoterCount:B.promoterCount,timeSlot:B.timeSlot,serviceDurationHours:B.serviceDurationHours,distributionLocation:B.distributionLocation,distributionPointType:B.distributionPointType,operationalNotes:B.operationalNotes,targetBusinessType:B.targetBusinessType,businessCategory:B.businessCategory,targetBusinessCount:B.targetBusinessCount,businessZone:B.businessZone,deliveryType:B.deliveryType}}),W=A=>i(F=>{const B=F.extraServices||[],P=B.includes(A)?B.filter(ge=>ge!==A):[...B,A],J={...F,extraServices:P},V=h[J.subscription]||1,H=f[J.subscription]||0,ue=J.subscription==="single"?1:J.campaignsPerMonth||1;return{...J,campaignsPerMonth:ue,selectedService:J.type,businessSector:J.activityType,flyerQuantity:J.qty,campaignPeriodStart:J.startDate,campaignPeriodEnd:J.endDate,alreadyPrinted:J.hasFlyers==="yes",printServices:P.filter(ge=>["stampa","grafica"].includes(ge)),paperWeight:J.printGramm,printSides:J.printSide,colorMode:J.printColor,campaignPlan:J.subscription,totalCampaigns:ue*V,planDiscount:H}});return _jsxs("div",{style:{maxWidth:960,margin:"0 auto",padding:"42px 28px 140px"},children:[_jsxs("div",{style:{marginBottom:32},children:[_jsx("h2",{style:{fontFamily:F.serif,fontSize:34,color:C.white,letterSpacing:"-1px",marginBottom:8},children:"Configura la campagna"}),_jsx("p",{style:{fontFamily:F.sans,fontSize:15,color:"rgba(255,255,255,.44)"},children:"Scegli il tipo di distribuzione. Il prezzo viene calcolato in Step 4 in base alla zona."}),_jsx("div",{style:{marginTop:14,padding:"12px 14px",borderRadius:12,background:"rgba(46,204,138,.07)",border:"1px solid rgba(46,204,138,.18)"},children:_jsxs("p",{style:{fontFamily:F.sans,fontSize:13,color:"rgba(255,255,255,.8)",lineHeight:1.55,margin:0},children:[_jsx("span",{style:{color:C.green,fontWeight:600},children:"Smart Pairing:"})," in Step 3 vedrai le date con sconto fino al −40%. Se non trovi il giorno, puoi inviare una richiesta. ",_jsx("button",{type:"button",onClick:()=>setShowSmartPairingModal(true),style:{color:C.green,textDecoration:"underline",textUnderlineOffset:2,background:"none",border:"none",cursor:"pointer",fontSize:13,fontFamily:F.sans,padding:0},children:"Scopri come funziona →"})]})})]}),_jsxs("div",{id:"section-servizio",style:{marginBottom:26},children:[_jsx(xn,{children:"1 – Tipo di distribuzione"}),_jsx("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))",gap:15,alignItems:"stretch"},children:u.map(({id:A,name:F,code:B,desc:P,features:J,c:V,popular:H})=>{const ue=n.type===A;return _jsxs("div",{role:"button",tabIndex:0,"aria-pressed":ue,onClick:()=>D({type:A}),onKeyDown:ge=>{(ge.key==="Enter"||ge.key===" ")&&(ge.preventDefault(),D({type:A}))},onFocus:ge=>{ge.currentTarget.style.boxShadow=`0 0 0 3px ${V}33`},onBlur:ge=>{ge.currentTarget.style.boxShadow=ue?`0 16px 36px ${V}18`:"none"},onMouseEnter:ge=>{ge.currentTarget.style.transform="translateY(-3px)",ge.currentTarget.style.borderColor=ue?C.orange:`${V}55`},onMouseLeave:ge=>{ge.currentTarget.style.transform="translateY(0)",ge.currentTarget.style.borderColor=ue?C.orange:"rgba(255,255,255,.08)"},style:{minHeight:390,borderRadius:14,padding:"24px 22px",cursor:"pointer",position:"relative",overflow:"hidden",display:"flex",flexDirection:"column",border:`2px solid ${ue?C.orange:"rgba(255,255,255,.08)"}`,background:ue?`linear-gradient(180deg,${V}13,rgba(255,255,255,.035))`:"rgba(255,255,255,.035)",transition:"transform.18s,border-color.18s,box-shadow.18s,background.18s",boxShadow:ue?`0 16px 36px ${V}18`:"none"},children:[_jsx("div",{style:{position:"absolute",inset:0,background:`linear-gradient(135deg,${V}12,transparent 46%)`,pointerEvents:"none",opacity:ue?1:.55}}),H&&_jsx("div",{style:{position:"absolute",top:14,right:14,background:C.orange,padding:"4px 9px",borderRadius:100,fontFamily:F.sans,fontSize:9,fontWeight:700,color:C.white,textTransform:"uppercase",letterSpacing:".08em",zIndex:1},children:"Popolare"}),ue&&_jsx("div",{style:{position:"absolute",top:14,left:14,width:22,height:22,borderRadius:"50%",background:C.orange,display:"flex",alignItems:"center",justifyContent:"center",zIndex:1,boxShadow:`0 0 0 4px ${C.orange}22`},children:_jsx("svg",{width:"10",height:"10",viewBox:"0 0 10 10",children:_jsx("path",{d:"M1.5 5l2.4 2.4L8.5 2.5",stroke:"white",strokeWidth:"1.7",fill:"none",strokeLinecap:"round",strokeLinejoin:"round"})})}),_jsxs("div",{style:{position:"relative",zIndex:1,display:"flex",alignItems:"center",gap:12,marginBottom:18,paddingLeft:ue?28:0},children:[_jsx("div",{style:{width:48,height:48,borderRadius:12,background:`${V}18`,border:`1px solid ${V}36`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:F.sans,fontSize:13,fontWeight:800,color:V,letterSpacing:".06em"},children:B}),_jsx("div",{style:{width:30,height:3,background:V,borderRadius:2}})]}),_jsx("div",{style:{position:"relative",zIndex:1,fontFamily:F.serif,fontSize:24,color:C.white,letterSpacing:"-.4px",lineHeight:1.08,marginBottom:10},children:F}),_jsx("p",{style:{position:"relative",zIndex:1,fontFamily:F.sans,fontSize:13,color:"rgba(255,255,255,.52)",lineHeight:1.62,marginBottom:18},children:P}),_jsx("div",{style:{position:"relative",zIndex:1,display:"flex",flexDirection:"column",gap:8,marginBottom:18},children:J.map(ge=>_jsxs("div",{style:{display:"flex",gap:9,alignItems:"flex-start"},children:[_jsx("div",{style:{width:15,height:15,borderRadius:5,background:ue?V:"rgba(255,255,255,.06)",border:`1px solid ${ue?V:"rgba(255,255,255,.1)"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1},children:_jsx("svg",{width:"8",height:"8",viewBox:"0 0 8 8",children:_jsx("path",{d:"M1.2 4.1l1.7 1.7 3.8-4",stroke:ue?"white":V,strokeWidth:"1.4",fill:"none",strokeLinecap:"round",strokeLinejoin:"round"})})}),_jsx("span",{style:{fontFamily:F.sans,fontSize:12,color:"rgba(255,255,255,.66)",lineHeight:1.35},children:ge})]},ge))}),_jsxs("div",{style:{position:"relative",zIndex:1,marginTop:"auto"},children:[_jsx("div",{style:{padding:"10px 12px",borderRadius:9,background:"rgba(255,255,255,.055)",border:"1px solid rgba(255,255,255,.08)",fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.45)",lineHeight:1.4,marginBottom:12},children:"Prezzo calcolato su zona, quantita e date operative"}),_jsxs("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,fontFamily:F.sans,fontSize:13,fontWeight:700,color:ue?C.orange:"rgba(255,255,255,.78)"},children:[_jsx("span",{children:"Configura servizio"}),_jsx("span",{"aria-hidden":"true",style:{fontSize:16,transform:ue?"translateX(2px)":"none",transition:"transform.18s"},children:""})]})]})]},A)})})]}),n.type==="d2d"&&_jsxs("div",{style:{marginBottom:22},children:[_jsx(xn,{children:"2 – Tipo di attivita del cliente"}),_jsxs(yr,{children:[_jsx("p",{style:{fontFamily:F.sans,fontSize:12,color:"rgba(255,255,255,.38)",marginBottom:12},children:"Aiuta il sistema a ottimizzare target, orari e suggerimenti AI."}),_jsxs("select",{value:n.activityType||"",onChange:A=>D({activityType:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:n.activityType?"white":"rgba(255,255,255,.4)",fontFamily:F.sans,fontSize:14,cursor:"pointer",appearance:"none"},children:[_jsx("option",{value:"",children:"Seleziona il settore..."}),Ov.map(A=>_jsx("option",{value:A.value,children:A.label},A.value))]}),n.activityType==="altro"&&_jsx("input",{type:"text",placeholder:"Descrivi l'attivita...",value:n.activityNote||"",onChange:A=>D({activityNote:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14,marginTop:10}})]})]}),m&&_jsxs("div",{style:{marginBottom:22},children:[_jsx(xn,{children:"2 – Configurazione Hand to Hand"}),_jsxs(yr,{children:[_jsxs("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:14,marginBottom:14},children:[_jsxs("div",{children:[_jsx("label",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)",display:"block",marginBottom:6},children:"Numero promoter"}),_jsxs("select",{value:n.promoterCount||"",onChange:A=>D({promoterCount:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14},children:[_jsx("option",{value:"",children:"Seleziona..."}),Bv.map(A=>_jsx("option",{value:A.value,children:A.label},A.value))]})]}),_jsxs("div",{children:[_jsx("label",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)",display:"block",marginBottom:6},children:"Fascia oraria"}),_jsxs("select",{value:n.timeSlot||"",onChange:A=>D({timeSlot:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14},children:[_jsx("option",{value:"",children:"Seleziona..."}),Mv.map(A=>_jsx("option",{value:A.value,children:A.label},A.value))]})]}),_jsxs("div",{children:[_jsx("label",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)",display:"block",marginBottom:6},children:"Durata servizio"}),_jsxs("select",{value:n.serviceDurationHours||"",onChange:A=>D({serviceDurationHours:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14},children:[_jsx("option",{value:"",children:"Seleziona..."}),Fv.map(A=>_jsx("option",{value:A.value,children:A.label},A.value))]})]})]}),_jsxs("div",{style:{display:"grid",gridTemplateColumns:l?"1fr":"1fr 1fr",gap:14,marginBottom:14},children:[_jsxs("div",{children:[_jsx("label",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)",display:"block",marginBottom:6},children:"Luogo principale"}),_jsx("input",{type:"text",placeholder:"es. Duomo, Stazione Centrale...",value:n.distributionLocation||"",onChange:A=>D({distributionLocation:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14}})]}),_jsxs("div",{children:[_jsx("label",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)",display:"block",marginBottom:6},children:"Tipo punto di distribuzione"}),_jsxs("select",{value:n.distributionPointType||"",onChange:A=>D({distributionPointType:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14},children:[_jsx("option",{value:"",children:"Seleziona..."}),Nv.map(A=>_jsx("option",{value:A.value,children:A.label},A.value))]})]})]}),_jsx("textarea",{placeholder:"Note operative (es. target studenti, uscita uffici...)",value:n.operationalNotes||"",onChange:A=>D({operationalNotes:A.target.value}),rows:2,style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14,resize:"vertical"}}),_jsxs("div",{style:{marginTop:12,fontFamily:F.sans,fontSize:12,color:C.blue,background:"rgba(96,165,250,.08)",padding:"8px 12px",borderRadius:8,border:"1px solid rgba(96,165,250,.2)"},children:[" ",_jsx("b",{children:"Hand to Hand:"})," il prezzo dipende da numero promoter, ore di servizio, luogo e quantita materiale."]})]})]}),y&&_jsxs("div",{style:{marginBottom:22},children:[_jsx(xn,{children:"2 – Configurazione Business Distribution"}),_jsxs(yr,{children:[_jsxs("div",{style:{display:"grid",gridTemplateColumns:l?"1fr":"1fr 1fr",gap:14,marginBottom:14},children:[_jsxs("div",{children:[_jsx("label",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)",display:"block",marginBottom:6},children:"Tipo attivita target"}),_jsxs("select",{value:n.targetBusinessType||"",onChange:A=>D({targetBusinessType:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14},children:[_jsx("option",{value:"",children:"Seleziona..."}),$v.map(A=>_jsx("option",{value:A.value,children:A.label},A.value))]})]}),_jsxs("div",{children:[_jsx("label",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)",display:"block",marginBottom:6},children:"Categoria commerciale"}),_jsxs("select",{value:n.businessCategory||"",onChange:A=>D({businessCategory:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14},children:[_jsx("option",{value:"",children:"Seleziona..."}),Lv.map(A=>_jsx("option",{value:A.value,children:A.label},A.value))]})]})]}),_jsxs("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:14,marginBottom:14},children:[_jsxs("div",{children:[_jsx("label",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)",display:"block",marginBottom:6},children:"Numero attivita target"}),_jsxs("select",{value:n.targetBusinessCount||"",onChange:A=>D({targetBusinessCount:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14},children:[_jsx("option",{value:"",children:"Seleziona..."}),Iv.map(A=>_jsx("option",{value:A.value,children:A.label},A.value))]})]}),_jsxs("div",{children:[_jsx("label",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)",display:"block",marginBottom:6},children:"Zona commerciale / comune"}),_jsx("input",{type:"text",placeholder:"es. zona uffici, area industriale...",value:n.businessZone||"",onChange:A=>D({businessZone:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14}})]}),_jsxs("div",{children:[_jsx("label",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)",display:"block",marginBottom:6},children:"Tipo consegna"}),_jsxs("select",{value:n.deliveryType||"",onChange:A=>D({deliveryType:A.target.value}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14},children:[_jsx("option",{value:"",children:"Seleziona..."}),km.map(A=>_jsx("option",{value:A.value,children:A.label},A.value))]})]})]}),_jsx("textarea",{placeholder:"Note operative (es. solo ristoranti, evitare catene...)",value:n.operationalNotes||"",onChange:A=>D({operationalNotes:A.target.value}),rows:2,style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14,resize:"vertical"}}),_jsxs("div",{style:{marginTop:12,fontFamily:F.sans,fontSize:12,color:C.purple,background:"rgba(167,139,250,.08)",padding:"8px 12px",borderRadius:8,border:"1px solid rgba(167,139,250,.2)"},children:[" ",_jsx("b",{children:"Business:"})," il prezzo dipende da attivita target, zona, quantita e tipo consegna."]})]})]}),_jsxs("div",{style:{display:"grid",gridTemplateColumns:l?"1fr":"1fr 1fr",gap:16,marginBottom:22},children:[_jsxs(yr,{id:"section-quantita",children:[_jsx(xn,{children:"2 – Quantita volantini"}),_jsx("div",{style:{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12},children:[5e3,1e4,25e3,5e4,1e5].map(A=>_jsx("button",{onClick:()=>D({qty:A}),style:{padding:"7px 13px",borderRadius:7,cursor:"pointer",fontFamily:F.sans,fontSize:12,fontWeight:600,border:`1px solid ${n.qty===A?C.orange:"rgba(255,255,255,.11)"}`,background:n.qty===A?"rgba(232,87,26,.15)":"rgba(255,255,255,.04)",color:n.qty===A?C.orange:"rgba(255,255,255,.58)",transition:"all.16s"},children:A.toLocaleString("it-IT")},A))}),_jsx("input",{type:"number",min:"1",value:n.qty||"",onChange:A=>D({qty:Math.max(0,+A.target.value||0)}),style:{width:"100%",padding:"10px 14px",borderRadius:9,border:"1px solid rgba(255,255,255,.11)",background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14}})]}),_jsxs(yr,{id:"section-periodo",children:[_jsx(xn,{children:"3 – Periodo campagna"}),_jsx("div",{style:{display:"grid",gridTemplateColumns:l?"1fr":"1fr 1fr",gap:10,marginBottom:10},children:[{l:"Data inizio",k:"startDate"},{l:"Data fine",k:"endDate"}].map(({l:A,k:F})=>{const B=n[F]||"",P=`${F}Draft`,J=n[P]??j(B);return _jsxs("div",{children:[_jsx("label",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.36)",display:"block",marginBottom:5},children:A}),_jsxs("div",{style:{position:"relative"},children:[_jsx("input",{type:"text",inputMode:"numeric",placeholder:"gg/mm/aaaa",value:J,onChange:V=>{const H=T(V.target.value),ue=z(H);D({[P]:H,...ue?{[F]:ue}:H?{}:{[F]:""}})},style:{width:"100%",padding:"10px 38px 10px 14px",borderRadius:9,border:`1px solid ${B?"rgba(232,87,26,.4)":"rgba(255,255,255,.12)"}`,background:"rgba(255,255,255,.06)",color:C.white,fontFamily:F.sans,fontSize:14,colorScheme:"dark"}}),_jsx("input",{type:"date",style:{position:"absolute",right:0,top:0,opacity:0,width:"100%",height:"100%",cursor:"pointer",pointerEvents:"none"},id:`picker-${F}`,value:B||"",onChange:V=>{const H=V.target.value;if(!H)return;
const[ue,ge,Fe]=H.split("-");D({[P]:`${Fe}/${ge}/${ue}`,[F]:H})}}),_jsx("span",{onClick:()=>{try{document.getElementById(`picker-${F}`).showPicker()}catch{document.getElementById(`picker-${F}`).click()}},style:{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",pointerEvents:"auto",cursor:"pointer",zIndex:10},children:React.createElement("svg",{width:15,height:15,viewBox:"0 0 16 16",fill:"none",xmlns:"http://www.w3.org/2000/svg",style:{display:"block",stroke:"rgba(255,255,255,.55)"}},React.createElement("rect",{x:1.5,y:2.5,width:13,height:12,rx:1.8,strokeWidth:1.2}),React.createElement("path",{d:"M5 1v3M11 1v3M1.5 6.5h13",strokeWidth:1.2,strokeLinecap:"round"}))})]}),B&&_jsxs("div",{style:{fontFamily:F.sans,fontSize:10,color:C.orange,marginTop:3},children:[" ",j(B)]})]},F)})}),_jsx("div",{style:{padding:"7px 11px",borderRadius:7,background:"rgba(255,255,255,.04)",border:`1px solid ${w?C.red:"rgba(255,255,255,.06)"}`,fontFamily:F.sans,fontSize:10,color:w?C.red:"rgba(255,255,255,.35)"},children:w?"Data fine precedente alla data inizio. Puoi correggerla ora o inviare una richiesta nello Step 3.":"Periodo indicativo. Nello Step 3 vedrai solo opportunita Smart Pairing; se non trovi il giorno desiderato potrai inviare una richiesta."})]})]}),_jsxs("div",{id:"section-formato",style:{marginBottom:22},children:[_jsx(xn,{children:"4 – Tipo di volantino"}),_jsxs(yr,{children:[_jsxs("div",{style:{marginBottom:16},children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:12,fontWeight:600,color:"rgba(255,255,255,.65)",marginBottom:10},children:"Hai gia i volantini stampati?"}),_jsx("div",{style:{display:"grid",gridTemplateColumns:l?"1fr":"1fr 1fr",gap:10},children:[{id:"yes",icon:"",label:"Si, li ho gia",sub:"Inserisci solo il formato"},{id:"no",icon:"",label:"No, devo stamparli",sub:"Aggiungi stampa nei servizi extra"}].map(({id:A,icon:F,label:B,sub:P})=>{const J=(n.hasFlyers||"no")===A;return _jsxs("div",{onClick:()=>D({hasFlyers:A,extraServices:A==="yes"?(n.extraServices||[]).filter(V=>!["stampa","grafica"].includes(V)):n.extraServices||[]}),style:{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",borderRadius:11,cursor:"pointer",border:`2px solid ${J?C.green:"rgba(255,255,255,.08)"}`,background:J?"rgba(46,204,138,.08)":"rgba(255,255,255,.03)",transition:"all.18s"},children:[_jsx("span",{style:{fontSize:20},children:F}),_jsxs("div",{children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:13,fontWeight:600,color:C.white,marginBottom:2},children:B}),_jsx("div",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)"},children:P})]})]},A)})})]}),_jsxs("div",{children:[_jsxs("div",{style:{fontFamily:F.sans,fontSize:11,fontWeight:600,color:"rgba(255,255,255,.45)",marginBottom:10},children:["Formato volantino ",_jsx("span",{style:{color:"rgba(255,255,255,.28)",fontWeight:400},children:"(per il calcolo della distribuzione)"})]}),_jsx("div",{style:{display:"grid",gridTemplateColumns:l?"repeat(2,1fr)":"repeat(4,1fr)",gap:7},children:Uv.map(({id:A,label:F,size:B})=>{const P=n.flyerFormat===A;return _jsxs("div",{onClick:()=>D({flyerFormat:A}),style:{borderRadius:9,padding:"11px",cursor:"pointer",textAlign:"center",border:`1px solid ${P?C.orange:"rgba(255,255,255,.08)"}`,background:P?"rgba(232,87,26,.1)":"rgba(255,255,255,.03)",transition:"all.16s"},children:[_jsx("div",{style:{fontFamily:F.serif,fontSize:18,color:P?C.orange:C.white},children:F}),_jsx("div",{style:{fontFamily:F.sans,fontSize:10,color:"rgba(255,255,255,.42)"},children:B})]},A)})})]}),n.hasFlyers==="no"&&_jsxs("div",{style:{marginTop:16,padding:"14px",borderRadius:11,background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.08)"},children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:10,fontWeight:700,color:"rgba(255,255,255,.32)",letterSpacing:".1em",textTransform:"uppercase",marginBottom:12},children:"Servizi extra – Produzione"}),_jsx("div",{style:{display:"grid",gridTemplateColumns:l?"1fr":"1fr 1fr",gap:10},children:[{id:"stampa",icon:"",label:"Stampa volantini",price:"da 29€/1.000 pz",desc:"Stampa professionale offset, consegna in 5gg",col:C.blue},{id:"grafica",icon:"",label:"Grafica & Design",price:"49€ flat",desc:"Design volantino professionale brandizzato",col:C.purple}].map(({id:A,icon:F,label:B,price:P,desc:J,col:V})=>{const H=(n.extraServices||[]).includes(A);return _jsxs("div",{onClick:()=>W(A),style:{padding:"13px 14px",borderRadius:10,cursor:"pointer",position:"relative",border:`2px solid ${H?C.orange:"rgba(255,255,255,.08)"}`,background:H?"rgba(232,87,26,.07)":"rgba(255,255,255,.03)",transition:"all.18s"},children:[_jsxs("div",{style:{display:"flex",alignItems:"center",gap:8,marginBottom:5},children:[_jsx("span",{style:{fontSize:18},children:F}),_jsxs("div",{style:{flex:1},children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:12,fontWeight:700,color:H?C.white:"rgba(255,255,255,.7)"},children:B}),_jsx("div",{style:{fontFamily:F.sans,fontSize:11,fontWeight:700,color:H?V:C.orange},children:P})]}),_jsx("div",{style:{width:18,height:18,borderRadius:5,border:`2px solid ${H?C.orange:"rgba(255,255,255,.2)"}`,background:H?C.orange:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},children:H&&_jsx("svg",{width:"9",height:"9",viewBox:"0 0 9 9",children:_jsx("path",{d:"M1.5 4.5l2 2 4-4",stroke:"white",strokeWidth:"1.5",fill:"none",strokeLinecap:"round"})})})]}),_jsx("div",{style:{fontFamily:F.sans,fontSize:10,color:"rgba(255,255,255,.38)"},children:J})]},A)})}),(n.extraServices||[]).includes("stampa")&&_jsxs("div",{style:{marginTop:12,padding:"14px",borderRadius:10,background:"rgba(96,165,250,.06)",border:"1px solid rgba(96,165,250,.2)"},children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:10,fontWeight:700,color:C.blue,letterSpacing:".08em",textTransform:"uppercase",marginBottom:12},children:"Specifiche stampa"}),_jsxs("div",{style:{marginBottom:12},children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.5)",marginBottom:7},children:"Grammatura carta"}),_jsx("div",{style:{display:"flex",gap:6,flexWrap:"wrap"},children:[{id:"80",l:"80g",desc:"Leggera"},{id:"115",l:"115g",desc:"Standard"},{id:"135",l:"135g",desc:"Spessa"},{id:"170",l:"170g",desc:"Premium"},{id:"300",l:"300g",desc:"Lusso"}].map(({id:A,l:F,desc:B})=>{const P=(n.printGramm||"115")===A;return _jsxs("div",{onClick:()=>D({printGramm:A}),style:{padding:"6px 12px",borderRadius:7,cursor:"pointer",textAlign:"center",border:`1px solid ${P?C.blue:"rgba(255,255,255,.1)"}`,background:P?"rgba(96,165,250,.15)":"rgba(255,255,255,.04)",transition:"all.15s"},children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:12,fontWeight:P?700:400,color:P?C.white:"rgba(255,255,255,.6)"},children:F}),_jsx("div",{style:{fontFamily:F.sans,fontSize:9,color:"rgba(255,255,255,.35)"},children:B})]},A)})})]}),_jsxs("div",{style:{marginBottom:12},children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.5)",marginBottom:7},children:"Lato di stampa"}),_jsx("div",{style:{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7},children:[{id:"fronte",l:"Fronte",icon:"",desc:"Solo fronte"},{id:"fronte_retro",l:"Fronte + Retro",icon:"",desc:"Doppia faccia"},{id:"bifacciale",l:"Bifacciale",icon:"",desc:"Alta qualita"}].map(({id:A,l:F,icon:B,desc:P})=>{const J=(n.printSide||"fronte")===A;return _jsxs("div",{onClick:()=>D({printSide:A}),style:{padding:"9px",borderRadius:8,cursor:"pointer",textAlign:"center",border:`1px solid ${J?C.blue:"rgba(255,255,255,.08)"}`,background:J?"rgba(96,165,250,.12)":"rgba(255,255,255,.03)",transition:"all.15s"},children:[_jsx("div",{style:{fontSize:16,marginBottom:3},children:B}),_jsx("div",{style:{fontFamily:F.sans,fontSize:11,fontWeight:J?700:400,color:J?C.white:"rgba(255,255,255,.6)"},children:F}),_jsx("div",{style:{fontFamily:F.sans,fontSize:9,color:"rgba(255,255,255,.35)"},children:P})]},A)})})]}),_jsxs("div",{children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.5)",marginBottom:7},children:"Tipo colore"}),_jsx("div",{style:{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7},children:[{id:"cmyk",l:"Colore CMYK",icon:"",desc:"Piena qualita colore"},{id:"bn",l:"Bianco e Nero",icon:"",desc:"Solo B/N, piu economico"},{id:"bn_colore",l:"B/N + Colore",icon:"",desc:"Testo B/N, loghi colore"}].map(({id:A,l:F,icon:B,desc:P})=>{const J=(n.printColor||"cmyk")===A;return _jsxs("div",{onClick:()=>D({printColor:A}),style:{padding:"9px",borderRadius:8,cursor:"pointer",textAlign:"center",border:`1px solid ${J?C.blue:"rgba(255,255,255,.08)"}`,background:J?"rgba(96,165,250,.12)":"rgba(255,255,255,.03)",transition:"all.15s"},children:[_jsx("div",{style:{fontSize:16,marginBottom:3},children:B}),_jsx("div",{style:{fontFamily:F.sans,fontSize:11,fontWeight:J?700:400,color:J?C.white:"rgba(255,255,255,.6)"},children:F}),_jsx("div",{style:{fontFamily:F.sans,fontSize:9,color:"rgba(255,255,255,.35)"},children:P})]},A)})})]}),_jsx("div",{style:{marginTop:10,padding:"7px 11px",borderRadius:7,background:"rgba(46,204,138,.06)",border:"1px solid rgba(46,204,138,.18)",fontFamily:F.sans,fontSize:10,color:C.green},children:" Specifiche stampa salvate – prezzo aggiornato in Step 4"})]}),(n.extraServices||[]).length>0&&_jsx("div",{style:{marginTop:10,padding:"8px 12px",borderRadius:8,background:"rgba(46,204,138,.06)",border:"1px solid rgba(46,204,138,.18)",fontFamily:F.sans,fontSize:11,color:C.green},children:" Servizi produzione aggiunti al preventivo in Step 4"})]}),_jsxs("div",{style:{marginTop:16,padding:"14px",borderRadius:11,background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.08)"},children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:10,fontWeight:700,color:"rgba(255,255,255,.32)",letterSpacing:".1em",textTransform:"uppercase",marginBottom:12},children:"Servizi operativi & Controllo"}),_jsx("div",{style:{display:"grid",gridTemplateColumns:l?"1fr":"1fr 1fr",gap:10},children:Kp.map(({id:A,icon:F,label:B,price:P,desc:J,col:V})=>{const H=(n.extraServices||[]).includes(A);return _jsxs("div",{onClick:()=>W(A),style:{padding:"13px 14px",borderRadius:10,cursor:"pointer",position:"relative",border:`2px solid ${H?C.orange:"rgba(255,255,255,.08)"}`,background:H?"rgba(232,87,26,.07)":"rgba(255,255,255,.03)",transition:"all.18s"},children:[_jsxs("div",{style:{display:"flex",alignItems:"center",gap:8,marginBottom:5},children:[_jsx("span",{style:{fontSize:18},children:F}),_jsxs("div",{style:{flex:1},children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:12,fontWeight:700,color:H?C.white:"rgba(255,255,255,.7)"},children:B}),_jsx("div",{style:{fontFamily:F.sans,fontSize:11,fontWeight:700,color:H?V:C.orange},children:P})]}),_jsx("div",{style:{width:18,height:18,borderRadius:5,border:`2px solid ${H?C.orange:"rgba(255,255,255,.2)"}`,background:H?C.orange:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},children:H&&_jsx("svg",{width:"9",height:"9",viewBox:"0 0 9 9",children:_jsx("path",{d:"M1.5 4.5l2 2 4-4",stroke:"white",strokeWidth:"1.5",fill:"none",strokeLinecap:"round"})})})]}),_jsx("div",{style:{fontFamily:F.sans,fontSize:10,color:"rgba(255,255,255,.38)"},children:J})]},A)})}),(n.extraServices||[]).some(A=>Kp.find(F=>F.id===A))&&_jsx("div",{style:{marginTop:10,padding:"8px 12px",borderRadius:8,background:"rgba(46,204,138,.06)",border:"1px solid rgba(46,204,138,.18)",fontFamily:F.sans,fontSize:11,color:C.green},children:" Servizi operativi attivati nel riepilogo finale"})]})]})]}),_jsxs("div",{id:"section-urgenza",style:{marginBottom:22},children:[_jsx(xn,{children:"5 – Urgenza distribuzione"}),_jsx("div",{style:{display:"grid",gridTemplateColumns:l?"1fr":"1fr 1fr",gap:11},children:[{id:"normal",icon:"",label:"Standard",sub:"5-7 giorni lavorativi"},{id:"urgent",icon:"",label:"Urgente",sub:"24-48h – maggiorazione applicata"}].map(({id:A,icon:F,label:B,sub:P})=>{const J=n.urgency===A;return _jsxs("div",{onClick:()=>D({urgency:A}),style:{display:"flex",alignItems:"center",gap:12,padding:"16px 18px",borderRadius:11,cursor:"pointer",border:`2px solid ${J?A==="urgent"?"#FF4444":C.green:"rgba(255,255,255,.08)"}`,background:J?A==="urgent"?"rgba(255,68,68,.08)":"rgba(46,204,138,.08)":"rgba(255,255,255,.03)",transition:"all.18s"},children:[_jsx("div",{style:{fontSize:22},children:F}),_jsxs("div",{style:{flex:1},children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:14,fontWeight:600,color:C.white,marginBottom:2},children:B}),_jsx("div",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)"},children:P})]}),_jsx("div",{style:{width:19,height:19,borderRadius:"50%",border:`2px solid ${J?A==="urgent"?"#FF4444":C.green:"rgba(255,255,255,.2)"}`,background:J?A==="urgent"?"#FF4444":C.green:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},children:J&&_jsx("svg",{width:"9",height:"9",viewBox:"0 0 9 9",children:_jsx("path",{d:"M1.5 4.5l2 2 4-4.5",stroke:"white",strokeWidth:"1.5",fill:"none",strokeLinecap:"round"})})})]},A)})})]}),_jsxs("div",{id:"section-piano",style:{marginBottom:32},children:[_jsx(xn,{children:"6 – Piano campagna"}),_jsx("div",{style:{display:"grid",gridTemplateColumns:l?"repeat(2,1fr)":"repeat(4,1fr)",gap:9,marginBottom:12},children:Gu.map(({id:A,label:F,icon:B,disc:P})=>{const J=n.subscription===A,qp=n.type==="h2h"?2.20:n.type==="b2b"||n.type==="business-distribution"?3.50:1.85,qr=P>0&&n.qty>0?Math.round((n.qty/1000)*qp*12*(P/100)):0;return _jsxs("div",{onClick:()=>D({subscription:A,campaignsPerMonth:A==="single"?1:n.campaignsPerMonth||1}),style:{borderRadius:11,padding:"16px 14px",cursor:"pointer",border:`2px solid ${J?C.orange:"rgba(255,255,255,.07)"}`,background:J?"rgba(232,87,26,.1)":"rgba(255,255,255,.03)",transition:"all.18s",textAlign:"center",position:"relative"},children:[P>0&&_jsxs("div",{style:{position:"absolute",top:-8,right:8,background:C.green,color:C.white,fontFamily:F.sans,fontSize:9,fontWeight:700,padding:"2px 7px",borderRadius:100},children:["-",P,"%"]}),_jsx("div",{style:{fontSize:22,marginBottom:8},children:B}),_jsx("div",{style:{fontFamily:F.sans,fontSize:12,fontWeight:700,color:J?C.white:"rgba(255,255,255,.65)"},children:F}),qr>0&&_jsx("div",{style:{fontFamily:F.sans,fontSize:11,color:C.green,fontWeight:700,marginTop:4},children:"~€"+qr+"/anno"})]},A)})}),n.subscription&&n.subscription!=="single"&&_jsxs("div",{style:{padding:"16px 18px",borderRadius:12,background:"rgba(232,87,26,.08)",border:"1px solid rgba(232,87,26,.2)"},children:[_jsxs("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12},children:[_jsxs("div",{children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:13,fontWeight:700,color:C.white,marginBottom:3},children:"Quante campagne al mese?"}),_jsxs("div",{style:{fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.45)"},children:[Gu.find(A=>A.id===n.subscription)?.label," – sconto ",Gu.find(A=>A.id===n.subscription)?.disc,"% sulla distribuzione"]})]}),_jsx("div",{style:{display:"flex",gap:8},children:[1,2,3].map(A=>{const F=(n.campaignsPerMonth||1)===A;return _jsx("button",{onClick:()=>D({campaignsPerMonth:A}),style:{width:56,height:56,borderRadius:12,border:`2px solid ${F?C.orange:"rgba(255,255,255,.15)"}`,background:F?"rgba(232,87,26,.2)":"rgba(255,255,255,.04)",color:F?C.orange:"rgba(255,255,255,.55)",fontFamily:F.serif,fontSize:28,cursor:"pointer",transition:"all.18s"},children:A},A)})})]}),n.campaignsPerMonth&&_jsxs("div",{style:{marginTop:10,fontFamily:F.sans,fontSize:11,color:"rgba(255,255,255,.4)"},children:[n.campaignsPerMonth," campagna",n.campaignsPerMonth>1?"e":""," al mese  - ",h[n.subscription]||1," mesi = ",_jsxs("b",{style:{color:C.orange},children:[R," campagne totali"]})]})]})]}),_jsx("div",{style:{display:"flex",justifyContent:"flex-end"},children:_jsx("button",{className:"vb",onClick:()=>{const sc=[{f:!n.type,i:"section-servizio"},{f:!n.qty||n.qty<1,i:"section-quantita"},{f:!n.flyerFormat||!n.hasFlyers,i:"section-formato"},{f:!n.urgency,i:"section-urgenza"},{f:!n.subscription,i:"section-piano"}];for(const{f:ok,i:si}of sc){if(ok){const el=document.getElementById(si);if(el){el.scrollIntoView({behavior:"smooth",block:"center"});el.style.outline="2px solid rgba(232,87,26,.55)";el.style.borderRadius="12px";setTimeout(()=>{el.style.outline="";el.style.borderRadius=""},2500);}return;}}r();},style:{padding:"15px 36px",borderRadius:12,border:"none",background:C.orange,color:C.white,fontFamily:F.sans,fontSize:16,fontWeight:700,cursor:"pointer",boxShadow:"0 8px 24px rgba(232,87,26,.32)"},children:"Scegli la zona"})}),showSmartPairingModal&&_jsx("div",{style:{position:"fixed",inset:0,zIndex:50,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"},children:_jsxs("div",{style:{position:"relative",width:"100%",maxWidth:420},children:[_jsx("div",{style:{position:"absolute",inset:0,background:"rgba(0,0,0,.6)"},onClick:()=>setShowSmartPairingModal(false)}),_jsxs("div",{style:{position:"relative",background:"#1E1D1B",border:"1px solid rgba(255,255,255,.1)",borderRadius:16,padding:24,zIndex:1},children:[_jsx("button",{onClick:()=>setShowSmartPairingModal(false),style:{position:"absolute",top:14,right:14,background:"none",border:"none",cursor:"pointer",color:"rgba(255,255,255,.38)",fontSize:20,lineHeight:1},children:"x"}),_jsxs("div",{style:{display:"flex",alignItems:"center",gap:8,marginBottom:16},children:[_jsx("span",{style:{width:8,height:8,borderRadius:"50%",background:C.green,display:"inline-block"}}),_jsx("h3",{style:{fontFamily:F.sans,fontWeight:600,fontSize:15,color:C.white,margin:0},children:"Come funziona Smart Pairing"})]}),_jsxs("div",{style:{display:"flex",flexDirection:"column",gap:12},children:[_jsxs("p",{style:{fontFamily:F.sans,fontSize:13,color:"rgba(255,255,255,.65)",lineHeight:1.65,margin:0},children:["Il calendario Smart Pairing ",_jsx("strong",{style:{color:"rgba(255,255,255,.85)"},children:"non e un calendario completo di disponibilita"}),". Mostra solo le opportunita di abbinamento gia pianificate."]}),_jsxs("p",{style:{fontFamily:F.sans,fontSize:13,color:"rgba(255,255,255,.65)",lineHeight:1.65,margin:0},children:["Nello Step 3 vedrai le date dove puoi abbinare la campagna, con uno ",_jsx("strong",{style:{color:C.green},children:"sconto fino al -40%"})," sul costo di distribuzione."]}),_jsx("p",{style:{fontFamily:F.sans,fontSize:13,color:"rgba(255,255,255,.65)",lineHeight:1.65,margin:0},children:"Se non trovi il giorno, puoi inviare una richiesta: ti avviseremo quando lavoriamo nella tua zona o vicino."})]}),_jsx("button",{onClick:()=>setShowSmartPairingModal(false),style:{marginTop:18,width:"100%",background:C.orange,color:C.white,border:"none",borderRadius:8,padding:"10px",fontFamily:F.sans,fontSize:13,fontWeight:600,cursor:"pointer"},children:"Capito"})]})]})})]})}
...
function apiToZones(apiData, city) {
  ...
  const totF = v.famiglie_stimate || 0;
  const totP = v.popolazione_stimata || 0;
  const totV = v.volantini_consigliati || 0;
  ...
  return items.map((c, idx) => {
    const pct = c.pct_copertura || Math.round(100 / nC);
    const ratio = pct / 100;
    // Use per-municipality values when available (more accurate than total * ratio)
    const vol = c.volantini_nel_raggio || Math.round(totV * ratio);
    const fam = c.households_total > 0 ? Math.round(c.households_total * ratio) : Math.round(vol / 1.1);
    const pop = c.population_total > 0 ? Math.round(c.population_total * ratio) : Math.round(totP * ratio);
    ...
    return {
      ...
      geometry_geojson: c.geometry_geojson || null,
    };
  });
}
...
function capToZone(capData, idx) {
  ...
  return {
    ...
    geometry_geojson: capData.geometry_geojson || null,
...
const [activeMapLayers, setActiveMapLayers] = useState(() => defaultLayerState(svcType));
  ...
  useEffect(() => {
    ...
    if (prevActiveZoneIdRef.current !== data.activeZoneId) {
      prevActiveZoneIdRef.current = data.activeZoneId;

      // Update local states
      setSearch(activeZone.cityName || "");
      setCity(activeZone.city || null);
      setRadius(activeZone.radius || 3);
      ...
      setData(prev => ({
        ...
        flyerQuantity: activeZone.assigned_flyers || 10000,
        flyerQuantityFromStep1: activeZone.assigned_flyers || 10000,
        cityName: activeZone.cityName || "",
        city: activeZone.city || null,
        radius: activeZone.radius || 3,
  ...
  }, [search, city, radius, selected, selectedCaps, capDataMap, manualAssignments, allocationMode, searchMode, activeMapLayers, data.activeZoneId]);
...
const { data: apiData, loading: apiLoading, error: apiError } = useServiceAnalysis(city?.lat, city?.lng, radius, svcType, selectedMunicipality);
const { sectors } = useSectors(city?.lat, city?.lng, radius, svcType);
const { pois }    = usePoi(city?.lat, city?.lng, radius, svcType);
...
const allowMockFallback = allowMockData && import.meta.env.VITE_ALLOW_MOCK_STEP2 === "true";
const apiZones = (apiData && !apiData.error && apiData.values) ? apiToZones(apiData, city) : null;
const mockFallbackZones = (allowMockFallback && city && !apiLoading && !apiZones) ? ZONE_DATA.map(z => {
    if (!GEO_DATA.find(c => c.id === z.id)) return null;
const dist = z.dist[city.id] ?? 999;
const pct = computePct(dist, radius, z.area);
    return pct > 0 ? {...z, dist, pct } : null;
  }).filter(Boolean) : null;
const zonesInRadius = apiZones || mockFallbackZones || [];
  ...
  useEffect(() => {
    ...
    } else if (!apiZones && mockFallbackZones && mockFallbackZones.length > 0) {
      const fullZones = mockFallbackZones.filter(z => z.pct === 100).map(z => z.id);
      setSelected(prev => [...new Set([...prev.filter(id => mockFallbackZones.some(z => z.id === id)),...fullZones])]);
    } else if (!apiZones && !mockFallbackZones) { setSelected([]); }
  }, [city?.id, radius, !!apiZones, !!mockFallbackZones]);
...
const serviceKpis = selZones.length > 0 ? {
    ...
    hotspotStrength: Math.round(selZones.reduce((a, z) => a + (h2hHotspotStrength(z) || 0), 0) / selZones.length),
    ...
    poi: selZones.reduce((a, z) => a + (Number(z.poi) || 0), 0),
    transitStops: selZones.reduce((a, z) => a + (Number(z.transitStops) || 0) + (Number(z.trainStations) || 0), 0),
    flowScore: selZones.length ? Math.round(selZones.reduce((a, z) => a + (Number(z.flowScore) || 0), 0) / selZones.length) : 0,
  ...
  } : { area: "0", hotspotStrength: 0, families: 0, pop: 0, population: 0, coverage: 0, recommendedFlyers: 0, selectedComuni: [], comuniCount: 0, poi: 0, transitStops: 0, flowScore: 0, businesses: 0, competitors: 0, commercialDensity: 0, clusters: 0, targetBusinesses: 0, familyIndex: null, reachScore: null, roiScore: null, confidenceScore: null };
const radiusInsightRows = zonesInRadius.map(z => ({
      ...
      detail: isResidentialStep2
        ? `${z.families.toLocaleString("it-IT")} famiglie`
        : isMovementStep2
          ? `${z.poi} POI – ${z.transitStops} fermate`
          : isBusinessStep2
            ? `${z.targetBiz} target – ${z.competitors} competitor`
            : `${zCap(z).toLocaleString("it-IT")} volantini`
    }));
  ...
  const aiAgg = selZones.length > 0 ? {
    ...
    hotspots: selZones.map(z => z.hotspots).filter(Boolean).join(" – ") || "-",
    ...
    strongZone: selZones.map(z => z.strongZone).filter(Boolean).join(" – ") || "-",
  ...
  const zonesWithCoords = zonesInRadius.map((z, i) => {
    ...
    return {
      ...
      color: getComuneColor(z.id), geometry: z.geometry_geojson || null,
  ...
  const mainTargetLabel = isResidentialStep2 ? "Famiglie stim." : isMovementStep2 ? "POI rilevanti" : "Attivita tot.";
  ...
  const h2hAttractionSummary = isMovementStep2 && selZones.length > 0 ? 
    (typeof Pv === 'function' ? Pv(selZones) : [
      {label: "POI rilevanti", value: selZones.reduce((a, z) => a + (z.poi || 0), 0), color: "#3B82F6"},
      {label: "Fermate / stazioni", value: selZones.reduce((a, z) => a + (z.transitStops || 0) + (z.trainStations || 0), 0), color: "#10B981"},
      {label: "Scuole / eventi", value: selZones.reduce((a, z) => a + (z.strongPts || 0), 0), color: "#F59E0B"},
      {label: "Attrattori locali", value: selZones.reduce((a, z) => a + (z.nearbyBiz || 0), 0), color: "#8B5CF6"}
    ]) : [];
    ...
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "34px clamp(16px, 4vw, 32px) 160px", background: C.navyMid, minHeight: "100vh", overflow: "visible" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 14, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {[{ id: "d2d", icon: " ", l: "Door to Door", c: C.orange }, { id: "h2h", icon: "", l: "Hand to Hand", c: C.blue }, { id: "b2b", icon: "", l: "Business", c: C.purple }].map(({ id, icon, l, c }) => (
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {[{ id: "distribuzione", icon: "", l: "Zona & Distribuzione" }, { id: "tematica", icon: "", l: "Mappa Tematica" }, { id: "admininfo", icon: " ", l: "AdminInfo" }].map(({ id, icon, l }) => (
        ...
        <div style={{ marginLeft: "auto", flexShrink: 0 }}>
          <div style={{ padding: "4px 10px", borderRadius: 6, background: `${col}18`, fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: col }}>
      ...
      <div style={{ marginBottom: 12, padding: "9px 14px", borderRadius: 10, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          <span style={{ fontFamily: F.sans, fontSize: 12, color: "rgba(255,255,255,.55)" }}>
            Stai configurando la zona. Tutti i dati dettagliati (KPI, ISTAT, AdminInfo, output servizi) saranno visibili nel
          </span>
          <span style={{ fontFamily: F.sans, fontSize: 12, fontWeight: 700, color: col }}> Riepilogo completo in Step 4  Preventivo</span>
          <span style={{ fontFamily: F.sans, fontSize: 12, color: "rgba(255,255,255,.55)" }}>.</span>
        </div>
        <div style={{ flexShrink: 0, padding: "3px 9px", borderRadius: 6, background: `${col}18`, border: `1px solid ${col}28`, fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: col }}>Step 4</div>
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        {searchMode !== "cap" && (
          <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
            ...
            {S2_RADII.map(r => (
              <button key={r} onClick={() => { setRadius(r); setSelected([]); }}
                style={{
                  padding: "7px 10px", borderRadius: 7, border: `1px solid ${radius === r ? C.orange : "rgba(255,255,255,.1)"}`,
                  background: radius === r ? "rgba(232,87,26,.18)" : "rgba(255,255,255,.04)",
                  color: radius === r ? C.orange : "rgba(255,255,255,.5)",
                  fontFamily: F.sans, fontSize: 11, fontWeight: radius === r ? 700 : 400, cursor: "pointer", transition: "all.15s"
                }}>
                {r < 1 ? `${r * 1000}m` : `${r}km`}
              </button>
            ))}
            <span style={{ fontFamily: F.serif, fontSize: 18, color: C.orange, marginLeft: 6 }}>{radius < 1 ? `${radius * 1000}m` : `${radius}km`}</span>
          </div>
        )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{
            ...
            <Step2Map
              ...
              settori={sectors}
              pois={pois}
              onLayerToggle={(id) => setActiveMapLayers(prev => ({ ...prev, [id]: !prev[id] }))}
              ...
              onSelectZone={(zoneId) => setData(prev => ({ ...prev, activeZoneId: zoneId }))}
            />
            ...
            {city && selZones.length > 0 && (
              <div style={{ position: "absolute", top: 10, right: 10, background: `${col}e0`, borderRadius: 6, padding: "4px 10px", fontFamily: F.sans, fontSize: 9, fontWeight: 700, color: C.white }}>
            ...
            {isBusinessStep2 && city && (
              <div style={{ position: "absolute", top: 10, left: 10, background: "rgba(8,15,30,.88)", borderRadius: 8, padding: "8px 10px", border: "1px solid rgba(255,255,255,.08)", maxWidth: 220 }}>
                ...
                {[
                  { c: targetBusinessMeta.color, l: "attivita target / categoria" },
            ...
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, border: `1px solid ${col}28`, overflow: "hidden" }}>
              <div style={{ padding: "12px 14px", borderBottom: "1px solid rgba(255,255,255,.06)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
                ...
                <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.35)" }}>
                  Budget: <b style={{ color: col }}>{flyerQuantityFromStep1.toLocaleString("it-IT")}</b> vol.
                </div>
              </div>
                    ...
                    return (
                      <div key={cap} style={{ borderRadius: 10, border: `1px solid ${col}35`, background: `${col}08`, padding: "10px 12px" }}>
                          ...
                          <div>
                            ...
                            {zone && (
                              <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.45)", paddingLeft: 24 }}>
                                {zone.municipalityName} – ~{zone.families?.toLocaleString("it-IT")} famiglie – {zone.area} km²
                              </div>
                            )}
                          </div>
                          <div style={{ textAlign: "right" }}>
                            <div style={{ fontFamily: F.sans, fontSize: 13, fontWeight: 700, color: C.white }}>{assigned.toLocaleString("it-IT")} pz.</div>
                            <div style={{ fontFamily: F.sans, fontSize: 9, color: "rgba(255,255,255,.3)" }}>consigliati {required.toLocaleString("it-IT")}</div>
                          </div>
                        </div>
                        ...
                        <div style={{ marginTop: 8, display: "flex", justifyContent: "flex-end" }}>
                          <button onClick={() => { setSelectedCaps(prev => prev.filter(c => c !== cap)); setCapDataMap(prev => { const n={...prev}; delete n[cap]; return n; }); }}
                            style={{ padding: "4px 10px", borderRadius: 7, border: "1px solid rgba(248,113,113,.3)", background: "rgba(248,113,113,.08)", color: C.red, fontFamily: F.sans, fontSize: 10, cursor: "pointer" }}>
                            Rimuovi CAP
                          </button>
                        </div>
                      </div>
          ...
          {searchMode !== "cap" && city && (
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, border: "1px solid rgba(255,255,255,.07)", overflow: "hidden" }}>
              ...
              <div style={{ padding: "10px 14px", background: "rgba(255,255,255,.02)", borderBottom: "1px solid rgba(255,255,255,.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.45)", lineHeight: 1.4, maxWidth: 400 }}>
                  {allocationMode === "auto"
                    ? "Il sistema distribuisce automaticamente i volantini partendo dalle zone piu vicine e piu coerenti."
                ...
                {allocationMode === "manual" && (
                  <div style={{ textAlign: "right" }}>
                    ...
                    <div style={{ fontFamily: F.sans, fontSize: 12, fontWeight: 700, color: isInvalid ? C.red : C.green }}>
                      {totalAssigned.toLocaleString("it-IT")} / {flyerQuantityFromStep1.toLocaleString("it-IT")}
                    </div>
                  </div>
                )}
              </div>
              ...
              <div style={{ padding: "10px", display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                {[...allZones].sort((a, b) => zCap(b) - zCap(a)).map(z => {
                  const sel = z.isCap ? selectedCaps.includes(z.postalCode) : selected.includes(z.id);
                  ...
                  return (
                    <div key={z.id} style={{
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "24px minmax(160px,1fr) 32px" : "24px 1fr 180px 120px 32px", gap: 12, alignItems: "center" }}>
                        {/* Checkbox */}
                        <div onClick={() => toggleZone(z.id)} style={{
                        ...
                        {/* Nome & Info */}
                        <div onClick={() => toggleZone(z.id)} style={{ cursor: "pointer", flex: 1 }}>
                          ...
                          <div style={{ fontFamily: F.sans, fontSize: 9, color: "rgba(255,255,255,.25)", marginTop: 2 }}>
                            {isResidentialStep2
                              ? `${z.families.toLocaleString("it-IT")} famiglie – ${z.pop.toLocaleString("it-IT")} ab. – ${z.area} km² – ${z.coverage}% nel raggio`
                              : isBusinessStep2
                              ? `${z.targetBiz} target – ${z.competitors} competitor – ${z.clusters} cluster – ${z.topCats}`
                              : isMovementStep2
                                ? `${z.poi} POI – ${z.transitStops} fermate – ${z.strongPts} hotspot – ${z.timeSlots}`
                        ...
                        {sel ? (
                          <div>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                              <span style={{ fontFamily: F.sans, fontSize: 9, fontWeight: 700, color: alloc.allocationStatus === "full" ? C.green : C.orange, textTransform: "uppercase", letterSpacing: ".04em" }}>
                                {alloc.allocationStatus === "full" ? "Copertura Totale" : "Copertura Parziale"}
                              </span>
                              <span style={{ fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: C.white }}>{Math.min(100, alloc.coveragePercent)}%</span>
                            </div>
                            <div style={{ height: 5, borderRadius: 3, background: "rgba(255,255,255,.08)", overflow: "hidden" }}>
                              <div style={{ width: `${Math.min(100, alloc.coveragePercent)}%`, height: "100%", background: alloc.allocationStatus === "full" ? C.green : C.orange, borderRadius: 3 }} />
                            </div>
                          </div>
                        ) : (
                          ...
                          {sel ? (
                            isManual ? (
                              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
                                ...
                                <div style={{ fontFamily: F.sans, fontSize: 8, color: "rgba(255,255,255,.3)", marginTop: 2 }}>di {alloc.requiredFlyers.toLocaleString("it-IT")}</div>
                              </div>
                            ) : (
                              <div>
                                {alloc.assignedFlyers === 0 ? (
                                  <div style={{ fontFamily: F.sans, fontSize: 9, color: "rgba(255,255,255,.32)", fontStyle: "italic", lineHeight: 1.4, textAlign: "right" }}>
                                    Incluso nel raggio,<br />non coperto dal budget attuale
                                  </div>
                                ) : (
                                  <>
                                    <div style={{ fontFamily: F.sans, fontSize: 13, fontWeight: 700, color: C.white }}>
                                      {alloc.assignedFlyers.toLocaleString("it-IT")}
                                    </div>
                                    <div style={{ fontFamily: F.sans, fontSize: 8, color: "rgba(255,255,255,.3)" }}>di {alloc.requiredFlyers.toLocaleString("it-IT")} necessari</div>
                                  </>
                                )}
                              </div>
                            )
                          ) : (
                            <div style={{ fontFamily: F.sans, fontSize: 11, color: "rgba(255,255,255,.15)" }}>{alloc.requiredFlyers.toLocaleString("it-IT")} pz</div>
                          )}
                        </div>
                    ...
                    isPartial ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        <div style={{ padding: "10px 14px", borderRadius: 10, background: "rgba(232,87,26,.08)", border: "1px solid rgba(232,87,26,.28)", fontFamily: F.sans, fontSize: 11, color: C.orange, lineHeight: 1.5 }}>
                          {isResidentialStep2 ? `Copertura parziale: hai ${flyerQuantityFromStep1.toLocaleString("it-IT")} vol., servono ${requiredFlyers.toLocaleString("it-IT")}. Puoi continuare con copertura parziale.` : "Copertura parziale: quantita insufficiente. Puoi continuare con copertura parziale."}
                        </div>
                        <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 8 }}>
                          <button onClick={() => setData(d => ({...d, qty: requiredFlyers, flyerQuantity: requiredFlyers }))}
                            ...
                            Aumenta a {requiredFlyers.toLocaleString("it-IT")} vol.
                          </button>
                          <button onClick={handleNext}
                            style={{ flex: 1, padding: "9px 12px", borderRadius: 8, background: "rgba(255,255,255,.08)", color: C.white, border: "1px solid rgba(255,255,255,.15)", fontFamily: F.sans, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>
                      ...
                      <div style={{ padding: "10px 14px", background: "rgba(46,204,138,.08)", border: "1px solid rgba(46,204,138,.2)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ fontFamily: F.sans, fontSize: 12, fontWeight: 700, color: C.green }}> quantita sufficiente per la copertura totale.</div>
                        {remainingFlyers > 0 && (
                          <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.4)" }}>Avanzano {remainingFlyers.toLocaleString("it-IT")} pz.</div>
                        )}
                      ...
                      {isInvalid ? (
                        <div style={{ background: "rgba(248,113,113,.1)", border: "1px solid rgba(248,113,113,.3)", borderRadius: 10, padding: "10px 14px" }}>
                          ...
                          <div style={{ fontFamily: F.sans, fontSize: 11, color: "rgba(255,255,255,.7)" }}>
                            Hai assegnato <b style={{ color: C.white }}>{totalAssigned.toLocaleString("it-IT")}</b> volantini, ma ne hai disponibili solo <b style={{ color: C.white }}>{flyerQuantityFromStep1.toLocaleString("it-IT")}</b>.
                            Riduci una zona o aumenta la quantita.
                          </div>
                        </div>
                      ) : (
                        <div style={{ background: "rgba(46,204,138,.08)", border: "1px solid rgba(46,204,138,.2)", borderRadius: 10, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          <div>
                            ...
                            <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.4)", marginTop: 2 }}>
                              {remainingFlyers > 0 ? `Rimanenti: ${remainingFlyers.toLocaleString("it-IT")} volantini` : "Tutti i volantini sono stati assegnati."}
                            </div>
                          </div>
                        </div>
                      )}
                      <div style={{ display: "flex", gap: 8 }}>
                        {isInvalid && (
                          <button onClick={() => setData(d => ({...d, qty: totalAssigned, flyerQuantity: totalAssigned }))}
                            ...
                            Aumenta quantita a {totalAssigned.toLocaleString("it-IT")}
                          </button>
                        )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          ...
          {city && (
            <div style={{ background: "rgba(255,255,255,.02)", borderRadius: 12, border: "1px solid rgba(255,255,255,.06)", overflow: "hidden" }}>
              <div onClick={() => setShowAiInsight(prev => !prev)}
                style={{
                  padding: "10px 14px",
                  background: "rgba(255,255,255,.02)",
                  borderBottom: showAiInsight ? "1px solid rgba(255,255,255,.05)" : "none",
                  display: "flex",
                ...
                <span style={{ fontFamily: F.sans, fontSize: 10, color: col, fontWeight: 700 }}>
                  {showAiInsight ? "Nascondi ▲" : "Espandi ▼"}
                </span>
              </div>
          ...
          <div style={{ background: `${col}10`, borderRadius: 12, padding: "14px 16px", border: `1px solid ${col}28` }}>
            ...
            <div style={{ fontFamily: F.serif, fontSize: 32, color: C.white, letterSpacing: "-1.5px", lineHeight: 1, marginBottom: 3, fontVariantNumeric: "tabular-nums" }}>
              {targetTotal.toLocaleString("it-IT")}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 10 }}>
              {(isResidentialStep2 ? [
                { l: "Area", v: search || "-" },
                { l: "Raggio", v: radius < 1 ? `${radius * 1000}m` : `${radius}km` },
                { l: "Superficie", v: serviceKpis?.area ? formatAreaKm2(serviceKpis.area) : "–" },
                { l: "Comuni", v: selZones.length },
                { l: "Inseriti", v: flyerQuantityFromStep1.toLocaleString("it-IT") },
                { l: "Copertura completa", v: requiredFlyers.toLocaleString("it-IT") + " vol." },
                { l: coverageStatus === "partial" ? "Mancanti" : "Rimanenti", v: (coverageStatus === "partial" ? missingFlyers : remainingFlyers).toLocaleString("it-IT") },
                { l: "Stato", v: coverageStatus === "sufficient" ? "Sufficiente" : coverageStatus === "empty" ? "Da selezionare" : "Copertura parziale" },
              ] : isMovementStep2 ? [
                { l: "Area", v: search || "-" },
                { l: "Raggio", v: radius < 1 ? `${radius * 1000}m` : `${radius}km` },
                { l: "Zone", v: selZones.length },
                { l: "Hotspot", v: (serviceKpis?.hotspotStrength ?? "–") + "/100" },
                { l: "Inseriti", v: flyerQuantityFromStep1.toLocaleString("it-IT") },
                ...
                { l: "Area", v: search || "-" },
                { l: "Raggio", v: radius < 1 ? `${radius * 1000}m` : `${radius}km` },
                { l: "Categoria", v: targetBusinessMeta?.label ?? "–" },
                { l: "Zone", v: selZones.length },
                { l: "Inseriti", v: flyerQuantityFromStep1.toLocaleString("it-IT") },
                { l: "Necessari", v: requiredFlyers.toLocaleString("it-IT") },
                { l: coverageStatus === "partial" ? "Mancanti" : "Rimanenti", v: (coverageStatus === "partial" ? missingFlyers : remainingFlyers).toLocaleString("it-IT") },
            ...
            {selZones.length > 0 && (
              <div style={{ padding: "8px 10px", borderRadius: 8, background: coverageStatus === "sufficient" ? "rgba(46,204,138,.08)" : "rgba(248,113,113,.08)", border: `1px solid ${coverageStatus === "sufficient" ? "rgba(46,204,138,.22)" : "rgba(248,113,113,.24)"}` }}>
                <div style={{ fontFamily: F.sans, fontSize: 11, fontWeight: 700, color: coverageStatus === "sufficient" ? C.green : C.red, marginBottom: 2 }}>
                  {coverageStatus === "sufficient" ? "quantita sufficiente" : "Volantini insufficienti"}
                </div>
                <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.52)", lineHeight: 1.45 }}>
                  {coverageStatus === "sufficient"
                    ? `quantita sufficiente per coprire le zone selezionate. Avanzano ${flyerSurplus.toLocaleString("it-IT")} volantini.`
                    : `Hai ${flyerQuantityFromStep1.toLocaleString("it-IT")} volantini, servono ${requiredFlyers.toLocaleString("it-IT")}. Mancano ${missingFlyers.toLocaleString("it-IT")} volantini.`}
                </div>
              </div>
            )}
          </div>

          {/* KPI Output — compact 2×2 grid, service-specific */}
            ...
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, border: "1px solid rgba(255,255,255,.07)", overflow: "hidden" }}>
              <div style={{ padding: "9px 14px", borderBottom: "1px solid rgba(255,255,255,.06)", display: "flex", alignItems: "center", gap: 7 }}>
                ...
                <span style={{ fontFamily: F.sans, fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,.32)", letterSpacing: ".1em", textTransform: "uppercase", flex: 1 }}>
                  {isResidentialStep2 ? "D2D" : isMovementStep2 ? "H2H" : isBusinessStep2 ? "B2B" : "Output"} – output chiave
                </span>
              </div>
              {selZones.length === 0 ? (
                ...
                <div style={{ padding: "8px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                  {(isResidentialStep2 ? [
                    { l: "Famiglie stimate", v: formatNumber(serviceKpis.families), c: col },
                    { l: "Copertura stimata", v: `${serviceKpis.coverage || 0}%`, c: C.green },
                    { l: "Volantini consigliati", v: formatNumber(serviceKpis.recommendedFlyers), c: C.white },
                    { l: "Comuni nel raggio", v: String(zonesInRadius.length), c: C.blue },
                  ] : isMovementStep2 ? [
                    { l: "POI rilevati", v: formatNumber(serviceKpis.poi), c: col },
                    { l: "Hotspot score", v: `${serviceKpis.hotspotStrength || 0}/100`, c: C.green },
                    { l: "Flusso potenziale", v: `${serviceKpis.flowScore || 0}/100`, c: C.white },
                    { l: "Comuni nel raggio", v: String(zonesInRadius.length), c: C.blue },
                  ] : isBusinessStep2 ? [
                    { l: "Attività rilevate", v: formatNumber(serviceKpis.businesses), c: col },
                    { l: "Competitor", v: formatNumber(serviceKpis.competitors), c: C.red },
                    { l: "Cluster commerciali", v: formatNumber(serviceKpis.clusters), c: C.white },
                    { l: "Target businesses", v: formatNumber(serviceKpis.targetBusinesses), c: C.green },
                  ] : [
                    { l: "Zone", v: String(selZones.length), c: col },
                    { l: "Comuni nel raggio", v: String(zonesInRadius.length), c: C.blue },
                  ]).map(({ l, v, c }) => (
                    ...
                    {(omiInfo.values || []).slice(0, omiExpanded ? undefined : 1).map((v, i) => (
                      <div key={i} style={{ padding: "6px 8px", borderRadius: 8, background: "rgba(232,87,26,.06)", border: "1px solid rgba(232,87,26,.14)", marginBottom: 4 }}>
                        ...
                        <div style={{ fontFamily: F.sans, fontSize: 13, fontWeight: 800, color: C.orange }}>
                          {v.min_value != null ? v.min_value.toLocaleString("it-IT") : "–"}{" – "}{v.max_value != null ? v.max_value.toLocaleString("it-IT") : "–"}
          ...
          {isResidentialStep2 && viewMode === "distribuzione" && (serviceKpis.families > 0 || zonesInRadius.length > 0) && (
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, border: "1px solid rgba(255,255,255,.07)", overflow: "hidden" }}>
              <div style={{ padding: "9px 14px", borderBottom: "1px solid rgba(255,255,255,.06)", display: "flex", alignItems: "center", gap: 7 }}>
                <div style={{ width: 16, height: 3, background: col, borderRadius: 2 }} />
              ...
              <div style={{ padding: "8px 10px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5 }}>
                {[
                  ...
                  { l: "Densità", v: serviceKpis.area && serviceKpis.pop ? `${formatNumber(Math.round(serviceKpis.pop / Number(serviceKpis.area)))} ab/km²` : "—" },
                  ...
                  <div key={l} style={{ padding: "6px 7px", borderRadius: 7, background: "rgba(255,255,255,.04)" }}>
                    ...
                    <div style={{ fontFamily: F.sans, fontSize: 11, fontWeight: 700, color: C.white }}>{v || "—"}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {isResidentialStep2 && viewMode === "distribuzione" && selZones.length > 0 && (
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, border: "1px solid rgba(255,255,255,.07)", overflow: "hidden" }}>
              <div style={{ padding: "9px 14px", borderBottom: "1px solid rgba(255,255,255,.06)", display: "flex", alignItems: "center", gap: 7 }}>
                ...
                <button onClick={() => setDetailExpanded(v => !v)} style={{ padding: "2px 7px", borderRadius: 5, border: `1px solid ${col}35`, background: "transparent", color: col, fontFamily: F.sans, fontSize: 9, cursor: "pointer" }}>{detailExpanded ? "Meno" : "Comuni"}</button>
              </div>
              <div style={{ padding: 9 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                  {residentialScores.map(s => <ScoreGauge key={s.l} label={s.l} value={s.v} color={s.c} />)}
                </div>
                {detailExpanded && residentialRadiusRows.length > 0 && (
              ...
              <div style={{ padding: "8px 10px" }}>
                ...
                {businessCategorySummary.slice(0, 3).map(cat => (
                  <div key={cat.label} style={{ display: "flex", alignItems: "center", gap: 7, padding: "4px 0", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                    ...
                    <span style={{ fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: cat.target ? targetBusinessMeta.color : "rgba(255,255,255,.42)" }}>{cat.count.toLocaleString("it-IT")}</span>
                  </div>
                ))}
          ...
          {isMovementStep2 && viewMode === "distribuzione" && selZones.length > 0 && (
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, border: "1px solid rgba(255,255,255,.07)", overflow: "hidden" }}>
              <div style={{ padding: "9px 14px", borderBottom: "1px solid rgba(255,255,255,.06)", display: "flex", alignItems: "center", gap: 7 }}>
                <div style={{ width: 16, height: 3, background: col, borderRadius: 2 }} />
              ...
              <div style={{ padding: "8px 10px" }}>
                {h2hAttractionSummary.map(row => (
                  <div key={row.label} style={{ display: "flex", alignItems: "center", gap: 7, padding: "4px 0", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                    ...
                    <span style={{ fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: C.white }}>{row.value.toLocaleString("it-IT")}</span>
                  </div>
                ))}
          ...
          {viewMode === "admininfo" && (
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, border: "1px solid rgba(255,255,255,.07)", overflow: "hidden" }}>
              <div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(255,255,255,.06)", fontFamily: F.sans, fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,.32)", letterSpacing: ".1em", textTransform: "uppercase" }}>
                {isResidentialStep2 ? "AdminInfo – Residential planning" : isMovementStep2 ? "AdminInfo – Operational planning" : isBusinessStep2 ? "AdminInfo – Commercial planning" : "AdminInfo – Zona selezionata"}
              </div>
              {!aiAgg
                ? <div style={{ padding: "18px", textAlign: "center", fontFamily: F.sans, fontSize: 12, color: "rgba(255,255,255,.28)" }}>Seleziona zone per vedere AdminInfo</div>
                      ...
                      <div key={group} style={{ borderRadius: 10, background: "rgba(255,255,255,.035)", border: "1px solid rgba(255,255,255,.06)", overflow: "hidden" }}>
                        ...
                        <div style={{ padding: "7px 10px" }}>
                          {serviceMeta.aiCats.filter(row => row.group === group).map((row, i) => {
                            const v = row.v(aiAgg);
const unavailable = !v || String(v).includes("undefined");
                            return (
                              <div key={i} style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "5px 0", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                                ...
                                <span style={{ fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: unavailable ? "rgba(255,255,255,.28)" : row.c === "green" ? C.green : C.white, textAlign: "right", maxWidth: 150 }}>{unavailable ? "dato non disponibile" : v}</span>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    <div style={{ padding: "9px 10px", borderRadius: 9, background: "rgba(232,87,26,.08)", border: "1px solid rgba(232,87,26,.2)", fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.58)", lineHeight: 1.45 }}>
                      Lettura residenziale: priorità a comuni con alta concentrazione famiglie, buona copertura cassette e quantita coerente con il fabbisogno stimato.
                    </div>
                  </div>
                ) : isResidentialStep2 ? (
                  <div style={{ padding: "10px", display: "grid", gap: 10 }}>
                    <div style={{ borderRadius: 10, background: "rgba(255,255,255,.035)", border: "1px solid rgba(255,255,255,.06)", overflow: "hidden" }}>
                      ...
                      <div style={{ padding: "7px 10px" }}>
                        {[
                          { l: "Famiglie stimate", v: formatNumber(serviceKpis.families || aiAgg.families), c: C.green },
                          { l: "Popolazione stimata", v: formatNumber(serviceKpis.population || aiAgg.pop), c: C.white },
                          { l: "Superficie analizzata", v: serviceKpis.area ? formatAreaKm2(serviceKpis.area) : "dato non disponibile", c: C.white },
                          { l: "Densità media", v: serviceKpis.area && (serviceKpis.population || aiAgg.pop) ? `${formatNumber(Math.round((serviceKpis.population || aiAgg.pop) / serviceKpis.area))} ab./km²` : "dato non disponibile", c: C.white },
                          { l: "Comuni nel raggio", v: formatNumber(zonesInRadius.length), c: C.white },
                        ].map(({ l, v, c }) => (
                          <div key={l} style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "5px 0", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                            ...
                            <span style={{ fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: v === "dato non disponibile" ? "rgba(255,255,255,.28)" : c, textAlign: "right", maxWidth: 170 }}>{v}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div style={{ borderRadius: 10, background: "rgba(255,255,255,.035)", border: "1px solid rgba(255,255,255,.06)", overflow: "hidden" }}>
                      ...
                      <div style={{ padding: "7px 10px" }}>
                        {demoLoading ? (
                          ...
                          <div key={l} style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "5px 0", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                            <span style={{ fontFamily: F.sans, fontSize: 9, color: "rgba(255,255,255,.38)" }}>{l}</span>
                            <span style={{ fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: v == null ? "rgba(255,255,255,.28)" : c, textAlign: "right", maxWidth: 170 }}>{v ?? "dato non disponibile"}</span>
                          </div>
                        ))}
                      </div>
                    </div>
...
const unavailable = !v || String(v).includes("undefined");
                            return (
                              <div key={i} style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "5px 0", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                                <span style={{ fontFamily: F.sans, fontSize: 9, color: "rgba(255,255,255,.38)" }}>{row.l}</span>
                                <span style={{ fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: unavailable ? "rgba(255,255,255,.28)" : C.white, textAlign: "right", maxWidth: 150 }}>{unavailable ? "dato non disponibile" : v}</span>
                              </div>
                            );
                          })}
                        </div>
...
const unavailable = !v || String(v).includes("undefined");
                    ...
                    <div style={{ padding: "9px 10px", borderRadius: 9, background: `${targetBusinessMeta.color}10`, border: `1px solid ${targetBusinessMeta.color}22`, fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.58)", lineHeight: 1.45 }}>
                      Lettura operativa: priorità alle zone con piu attivita target, cluster forti e densità commerciale alta. I competitor sono contesto decisionale, non blocco automatico.
                    </div>
                  </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                      {[["0-14", aiAgg.eta14, "#93C5FD"], ["15-34", aiAgg.eta34, "#34D399"], ["35-64", aiAgg.eta64, "#FBBF24"], ["65+", aiAgg.eta65, "#F87171"]].map(([l, v, c]) => (
                        <div key={l} style={{ display: "flex", gap: 5, alignItems: "center" }}>
                          ...
                          <span style={{ fontFamily: F.sans, fontSize: 9, color: "rgba(255,255,255,.5)" }}>{l}: <b style={{ color: v != null ? C.white : "rgba(255,255,255,.28)" }}>{v != null ? `${v}%` : "N/D"}</b></span>
                        </div>
                      ))}
                    </div>
                  </div>
                  {[
                    { l: "Reddito medio", v: "EUR " + aiAgg.reddito.toLocaleString("it-IT"), c: C.green },
                    { l: "Tasso occupaz.", v: aiAgg.occup + "%", c: C.blue },
                    { l: "% Stranieri", v: aiAgg.stranieri + "%", c: C.teal },
                    { l: "Imprese tot.", v: aiAgg.imprese.toLocaleString("it-IT"), c: C.purple },
                  ].map(({ l, v, c }) => (
...
const unavailable = !v || String(v).includes("undefined");
                      return (
                        <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "4px 8px", borderRadius: 6, background: "rgba(255,255,255,.04)" }}>
                          ...
                          <span style={{ fontFamily: F.sans, fontSize: 10, fontWeight: 600, color: unavailable ? "rgba(255,255,255,.28)" : row.c === "green" ? C.green : C.white, textAlign: "right", maxWidth: 140 }}>{unavailable ? "dato non disponibile" : v}</span>
                        </div>
                      );
                    })}
                  </div>
            ...
            {showAdv && (
              <div style={{ padding: "10px" }}>
                {!firstZ
                  ...
                  : <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                    {(isResidentialStep2 ? residentialScores : isMovementStep2 ? [{ l: "Hotspot Strength", v: serviceKpis.hotspotStrength, c: C.blue },...h2hScores] : isBusinessStep2 ? [{ l: "Commercial Density Index", v: serviceKpis.cdIdx, c: C.purple },...businessScores] : cfg.advKpis(firstZ)).map((s, i) => <ScoreGauge key={i} label={s.l} value={s.v} color={s.c} />)}
                  </div>
                }
              </div>
            )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: "auto" }}>
            ...
            <button className="btn" onClick={handleNext} disabled={!((data.campaignZones || []).length > 0 && (data.campaignZones || []).every(z => (z.searchMode === "cap" ? (z.selectedCaps && z.selectedCaps.length > 0) : (z.city !== null)) && (z.assigned_flyers || 0) > 0))}
              style={{ width: "100%", minHeight: 52, padding: "0 14px", borderRadius: 11, border: "none", background: ((data.campaignZones || []).length > 0 && (data.campaignZones || []).every(z => (z.searchMode === "cap" ? (z.selectedCaps && z.selectedCaps.length > 0) : (z.city !== null)) && (z.assigned_flyers || 0) > 0)) ? col : "rgba(255,255,255,.1)", color: C.white, fontFamily: F.sans, fontSize: 14, fontWeight: 800, cursor: ((data.campaignZones || []).length > 0 && (data.campaignZones || []).every(z => (z.searchMode === "cap" ? (z.selectedCaps && z.selectedCaps.length > 0) : (z.city !== null)) && (z.assigned_flyers || 0) > 0)) ? "pointer" : "not-allowed", boxShadow: ((data.campaignZones || []).length > 0 && (data.campaignZones || []).every(z => (z.searchMode === "cap" ? (z.selectedCaps && z.selectedCaps.length > 0) : (z.city !== null)) && (z.assigned_flyers || 0) > 0)) ? `0 6px 18px ${C.orangeGlow}` : "none", textAlign: "center" }}>
              {((data.campaignZones || []).length > 0 && (data.campaignZones || []).every(z => (z.searchMode === "cap" ? (z.selectedCaps && z.selectedCaps.length > 0) : (z.city !== null)) && (z.assigned_flyers || 0) > 0)) ? "Continua al calendario" : "Seleziona/configura le zone"}
            </button>
            {!((data.campaignZones || []).length > 0 && (data.campaignZones || []).every(z => (z.searchMode === "cap" ? (z.selectedCaps && z.selectedCaps.length > 0) : (z.city !== null)) && (z.assigned_flyers || 0) > 0)) && (
            ...
            <div style={{ fontFamily: F.sans, fontSize: 8, color: "rgba(255,255,255,.18)", textAlign: "center" }}>
              Step 2 → Step 3. Scegli le date disponibili per ciascuna delle zone configurate. Nessun pagamento ora.
            </div>
          </div>
        </div>
      )}
    ...
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 28px 130px" }}>
      <div style={{ marginBottom: 22 }}>
        ...
        <p style={{ fontFamily: F.sans, fontSize: 14, color: "rgba(255,255,255,.48)", lineHeight: 1.65, maxWidth: 760 }}>
          Il calendario mostra opportunita Smart Pairing solo quando arrivano da disponibilita reali del backend. Se non ci sono slot confermati, puoi inviare una richiesta di disponibilita.
        </p>
      </div>
        ...
        <div>
          {isContinuativePlan && (
            <div style={{ marginBottom: 14, padding: "13px 15px", borderRadius: 12, background: "rgba(232,87,26,.08)", border: "1px solid rgba(232,87,26,.22)", display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3,1fr)", gap: 10 }}>
              ...
              <div style={{ gridColumn: "1 / -1", fontFamily: F.sans, fontSize: 11, color: "rgba(255,255,255,.5)", lineHeight: 1.45 }}>
                Per piani continuativi, lo Smart Pairing mostra solo abbinamenti compatibili confermati dal backend. Se non trovi slot adatti, invia una richiesta e il team ti avvisera quando ci sono campagne compatibili.
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 14, marginBottom: 14, flexWrap: "wrap" }}>
            {[
              ...
              { c: "rgba(255,255,255,.08)", l: "Nessun colore: disponibilita non configurata" },
            ].map(({ c, l }) => (
          ...
          {realSmartPairingSlots.length === 0 && (
            <div style={{ marginBottom: 14, padding: "12px 14px", borderRadius: 10, background: "rgba(251,191,36,.08)", border: "1px solid rgba(251,191,36,.2)", fontFamily: F.sans, fontSize: 12, color: "rgba(255,255,255,.62)", lineHeight: 1.55 }}>
              Disponibilita reale non ancora configurata. Puoi inviare una richiesta. Nessuno slot Smart Pairing disponibile per questa zona nelle date selezionate.
            </div>
          )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
          <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, padding: "16px", border: "1px solid rgba(255,255,255,.08)" }}>
            ...
            <div style={{ fontFamily: F.sans, fontSize: 12, color: "rgba(255,255,255,.5)", lineHeight: 1.7 }}>
              ...
              <b style={{ color: C.white }}>Stato:</b> {pairingDays.length ? "Smart Pairing selezionato" : "nessuna opportunita selezionata"}
            </div>
            {selDays.length === 0 ? <div style={{ fontFamily: F.sans, fontSize: 12, color: "rgba(255,255,255,.3)", textAlign: "center", padding: "14px 0" }}>Richiedi disponibilita per questa zona.</div> :
          ...
          <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, padding: "16px", border: "1px solid rgba(255,255,255,.08)" }}>
            ...
            <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.34)", lineHeight: 1.55 }}>Il prezzo finale viene calcolato nello Step 4 in base a servizio, zona, quantita e date.</div>
          </div>
          ...
          <div style={{ padding: "11px 13px", borderRadius: 10, background: "rgba(251,191,36,.06)", border: "1px solid rgba(251,191,36,.16)", fontFamily: F.sans, fontSize: 11, color: "rgba(255,255,255,.5)", lineHeight: 1.45 }}>
            Disponibilita reale non ancora configurata. Puoi inviare una richiesta per questa zona.
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 7, marginTop: "auto" }}>
            ...
            <button className="btn" onClick={handleSkipPairing} style={{ width: "100%", padding: "11px", borderRadius: 10, border: "1px solid rgba(255,255,255,.14)", background: "rgba(255,255,255,.035)", color: "rgba(255,255,255,.66)", fontFamily: F.sans, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Continua con richiesta disponibilita</button>
...
const normalizeSelectedExtras = (data) => {
    const mapping = [
      { id: "tracking_gps", oldIds: ["gps", "tracking_gps"], l: "Tracking GPS", d: "Monitoraggio operativo della distribuzione con tracciamento delle attivita.", icon: "", p: 25 },
      ...
      { id: "advanced_report", oldIds: ["report", "advanced_report", "report_avanzato"], l: "Report avanzato", d: "Report finale piu dettagliato con riepilogo operativo e indicatori principali.", icon: " ", p: 19 },
      ...
      { id: "quality_control", oldIds: ["quality", "quality_control", "controllo_qualita"], l: "Controllo qualita", d: "Verifica aggiuntiva sulla corretta esecuzione della distribuzione.", icon: "", p: 25 },
...
const d2dScores = [
    ...
    { l: "ROI Score", v: kpis.roiScore ?? (selZ.length ? Math.round(selZ.reduce((a, z) => a + z.roiD2D, 0) / selZ.length) : 0), c: C.green, d: "Coerenza costo/opportunita", src: "Analisi interna" },
...
const serviceSummaryConfig = {
    d2d: {
      ...
      fields: [
        ...
        remainingQty > 0 ? { l: "quantita residua", v: formatNumber(remainingQty), src: "Calcolo", c: C.green } : missingQty > 0 ? { l: "quantita mancante", v: formatNumber(missingQty), src: "Calcolo", c: C.red } : null,
      ],
      scores: d2dScores,
      admin: [
        { l: "Eta 0-14", v: (() => { const vs = selZ.map(z => z.eta14).filter(n => n != null); return vs.length ? `${Math.round(vs.reduce((a,n) => a+n, 0)/vs.length*10)/10}%` : null; })() },
        { l: "Eta 15-34", v: (() => { const vs = selZ.map(z => z.eta34).filter(n => n != null); return vs.length ? `${Math.round(vs.reduce((a,n) => a+n, 0)/vs.length*10)/10}%` : null; })() },
        { l: "Eta 35-64", v: (() => { const vs = selZ.map(z => z.eta64).filter(n => n != null); return vs.length ? `${Math.round(vs.reduce((a,n) => a+n, 0)/vs.length*10)/10}%` : null; })() },
        { l: "Eta 65+", v: (() => { const vs = selZ.map(z => z.eta65).filter(n => n != null); return vs.length ? `${Math.round(vs.reduce((a,n) => a+n, 0)/vs.length*10)/10}%` : null; })() },
    ...
    h2h: {
      ...
      fields: [
        ...
        { l: "Densita passaggio", v: kpis.passageDensity != null ? `${kpis.passageDensity}/100` : null, src: "Analisi interna", c: C.orange },
      ...
      admin: [
        ...
        { l: "Attivita vicine", v: kpis.nearbyBiz ? formatNumber(kpis.nearbyBiz) : null },
    ...
    b2b: {
      ...
      fields: [
        { l: "Attivita rilevate", v: formatNumber(kpis.businesses), src: "Google Places", c: C.purple },
        ...
        { l: "Attivita target", v: formatNumber(kpis.targetBusinesses), src: "Google Places", c: C.green },
      ],
      scores: [
      ...
      admin: [
        ...
        { l: "Zona piu forte", v: kpis.strongestZone },
...
const quotePdfData = {
    ...
    campaign: {
      ...
      materialStatus: alreadyPrinted ? "gia stampato" : "Da produrre",
    ...
    quantityExplanation: quantityIsSufficient
      ? "La quantita consigliata copre l'area selezionata. Eventuali volantini residui possono essere usati per ampliare il raggio, aggiungere comuni vicini o mantenere una scorta operativa."
      : `La quantita inserita non copre completamente l'area selezionata. Mancano ${missingQty.toLocaleString("it-IT")} volantini per raggiungere la copertura stimata.`,
    ...
    pricing: {
      lines: [{ label: `Distribuzione ${tLabel}`, detail: `${flyerQty.toLocaleString("it-IT")} volantini  - ${eur4(unitPricePerFlyer)}`, quantity: flyerQty, unitPrice: unitPricePerFlyer, total: baseCost }],
      subtotal: baseCost,
...
function handleDownloadPdf() {
  ...
  async function handleConfirmCampaign() {
    ...
    setSent(true);
    setConfirmSyncStatus(hasSupabaseConfig() ? "Salvataggio campagna in corso..." : "Backend non configurato: puoi scaricare il PDF o inviare una richiesta disponibilita.");
    if (!hasSupabaseConfig()) {
      return;
    }
    try {
    ...
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "28px 24px 140px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 22, flexWrap: "wrap" }}>
        <div style={{ flex: 1 }}>
          ...
          {isQuick && <p style={{ fontFamily: F.sans, fontSize: 13, color: "rgba(255,255,255,.45)", marginTop: 6 }}>Questo preventivo e stato generato con dati parziali. Il prezzo finale puo variare dopo l'analisi zona completa.</p>}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 290px", gap: 16, paddingBottom: isMobile ? 96 : 0 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{...box(), padding: "18px" }}>
            {secHead("1", "Tipo campagna", "Servizio, volantino, quantita, piano", C.orange)}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(155px,1fr))", gap: 8 }}>
              {nonEmpty([
                ...
                { icon: "", l: "quantita", v: flyerQty.toLocaleString("it-IT") + " pz.", c: C.white },
                { icon: "", l: "Formato", v: (data.flyerFormat || data.format || "-").toUpperCase(), c: C.green },
                { icon: "", l: "Materiale", v: alreadyPrinted ? "gia stampato" : "Da produrre", c: alreadyPrinted ? C.green : C.blue },
                !alreadyPrinted && { icon: "", l: "Grammatura", v: flyW, c: C.green },
                { icon: "", l: "Grafica", v: data.graphicsReady === true || data.designReady === true ? "File gia disponibile" : data.needGraphic || productionServices.includes("grafica") ? "File da preparare" : null, c: C.purple },
                ...
                (data.subscription && data.subscription !== "single") && { icon: "", l: "quantita/campagna", v: flyerQty.toLocaleString("it-IT") + " pz.", c: C.white },
          ...
          <div style={{...box(), padding: "18px" }}>
            ...
            {isQuick ? (
              <div style={{ padding: "14px", borderRadius: 10, background: "rgba(255,255,255,.025)", border: "1px solid rgba(255,255,255,.06)", fontFamily: F.sans, fontSize: 12, color: "rgba(255,255,255,.42)", lineHeight: 1.55 }}>
                Preventivo rapido: il dettaglio completo di KPI, comuni e AdminInfo sar  disponibile dopo l'analisi zona completa.
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(145px,1fr))", gap: 8 }}>
                    {nonEmpty([
                      ...
                      { l: "Modalita", v: data.areaMode === "cap" ? "CAP" : data.areaMode === "address-radius" ? "Indirizzo + raggio" : "Comune con raggio di analisi", src: "Step 2", c: col },
                ...
                <div>
                  ...
                  {svcType === "d2d" && requiredQty > 0 && (
                    <div style={{ marginTop: 8, padding: "10px 11px", borderRadius: 10, background: quantityIsSufficient ? "rgba(46,204,138,.08)" : "rgba(248,113,113,.08)", border: `1px solid ${quantityIsSufficient ? "rgba(46,204,138,.2)" : "rgba(248,113,113,.22)"}` }}>
                      <div style={{ fontFamily: F.sans, fontSize: 11, fontWeight: 800, color: quantityIsSufficient ? C.green : C.red, marginBottom: 4 }}>{quantityIsSufficient ? "quantita sufficiente" : "quantita insufficiente"}</div>
                      <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.58)", lineHeight: 1.5 }}>
                        {quantityIsSufficient
                          ? "La quantita consigliata copre l'area selezionata. I volantini residui possono essere usati per ampliare il raggio, aggiungere comuni vicini o mantenere una scorta operativa."
                          : "La quantita inserita non copre tutta l'area stimata. Puoi aumentare la quantita oppure procedere con copertura parziale se supportata dal piano operativo."}
                      </div>
                    </div>
                  )}
                </div>
                      ...
                      {breakdownRows.map(row => {
                        return (
                          <div key={row.id} style={{ padding: "9px 10px", borderRadius: 9, background: row.selectedRow ? "rgba(255,255,255,.04)" : "rgba(255,255,255,.02)", border: `1px solid ${row.selectedRow ? "rgba(255,255,255,.07)" : "rgba(255,255,255,.04)"}`, display: "grid", gridTemplateColumns: "1fr auto auto auto", gap: 10, alignItems: "center" }}>
                            ...
                            <div style={{ fontFamily: F.sans, fontSize: 11, fontWeight: 800, color: row.selectedRow ? col : "rgba(255,255,255,.36)" }}>{row.estimatedFlyers != null ? `${row.estimatedFlyers.toLocaleString("it-IT")} volantini` : "Estensione"}</div>
                ...
                <div>
                  <div style={{ fontFamily: F.sans, fontSize: 9, fontWeight: 800, color: "rgba(255,255,255,.34)", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 8 }}>AdminInfo / ISTAT summary</div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(145px,1fr))", gap: 8 }}>
          ...
          <div style={{...box(), padding: "18px" }}>
            {secHead("4", "Pianificazione campagna", "Date selezionate e opportunita disponibili", C.green)}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 8, marginBottom: 4 }}>
                      {nonEmpty([
                        ...
                        { l: "Disponibilita", v: disc > 0 ? "opportunita backend confermata" : "Richiesta disponibilita", src: "Step 3", c: disc > 0 ? C.green : C.white },
          ...
          <div style={{...box(), padding: "18px", position: isMobile ? "static" : "sticky", top: 140 }}>
            ...
            <div style={{ padding: "10px 11px", borderRadius: 10, background: "rgba(255,255,255,.035)", border: "1px solid rgba(255,255,255,.06)", marginBottom: 12 }}>
              ...
              <div style={{ fontFamily: F.sans, fontSize: 11, color: "rgba(255,255,255,.48)", lineHeight: 1.55 }}>
                {flyerQty.toLocaleString("it-IT")} volantini  - {eur4(unitPricePerFlyer)} = <b style={{ color: C.white }}>{eur(baseCost)}</b>
              </div>
            </div>
...
const remaining = campaign.qty - required;
    return {
      zones,
      rows: [
        { l: "Famiglie stimate", v: families.toLocaleString("it-IT") },
        { l: "Popolazione stimata", v: zones.reduce((a, z) => a + z.pop, 0).toLocaleString("it-IT") },
        { l: "Copertura stimata", v: `${Math.round(zones.reduce((a, z) => a + z.coverage, 0) / zones.length)}%` },
        { l: "Volantini consigliati", v: required.toLocaleString("it-IT") },
        { l: "Volantini inseriti", v: campaign.qty.toLocaleString("it-IT") },
        { l: remaining >= 0 ? "quantita residua" : "quantita mancante", v: Math.abs(remaining).toLocaleString("it-IT") },
  ...
  if (campaign.svc === "h2h") {
    return {
      zones,
      rows: [
        { l: "POI rilevanti", v: zones.reduce((a, z) => a + z.poi, 0).toLocaleString("it-IT") },
        { l: "Fermate / stazioni", v: zones.reduce((a, z) => a + z.transitStops + z.trainStations, 0).toLocaleString("it-IT") },
        { l: "Scuole / eventi", v: zones.reduce((a, z) => a + (z.strongPts || 0), 0).toLocaleString("it-IT") },
        { l: "Flusso potenziale", v: `${Math.round(zones.reduce((a, z) => a + z.flowScore, 0) / zones.length)}/100` },
        { l: "Densita passaggio", v: `${Math.round(zones.reduce((a, z) => a + z.commDens, 0) / zones.length)}/100` },
  ...
  return {
    zones,
    rows: [
      { l: "Attivita rilevate", v: zones.reduce((a, z) => a + z.bizTotal, 0).toLocaleString("it-IT") },
      { l: "Categorie principali", v: [...new Set(zones.flatMap(z => (z.topCats || "").split(" – ").filter(Boolean)))].join(" – ") },
      { l: "Competitor vicini", v: zones.reduce((a, z) => a + z.competitors, 0).toLocaleString("it-IT") },
      { l: "Cluster commerciali", v: zones.reduce((a, z) => a + z.clusters, 0).toLocaleString("it-IT") },
      { l: "Attivita target", v: zones.reduce((a, z) => a + z.targetBiz, 0).toLocaleString("it-IT") },
        ...
        <div style={{...box(), padding: "18px", marginBottom: 18, border: "1px solid rgba(232,87,26,.25)", background: "rgba(232,87,26,.05)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", gap: 10, alignItems: "flex-end" }}>
            {[
              ...
              { l: "quantita", type: "number", key: "qty", ph: "10000" },
            ].map(({ l, type, key, ph }) => (
              <div key={key}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{...box(), overflow: "hidden" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "70px 1fr 82px 118px 80px 90px 116px 96px 84px", gap: 0, padding: "8px 16px", borderBottom: "1px solid rgba(255,255,255,.05)" }}>
              {["ID", "Cliente", "Servizio", "Zona", "Quantita", "Totale EUR", "Pagamento", "Stato", "Analisi"].map(h => (
              ...
              return (
                <div key={c.id} style={{ borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "70px 1fr 82px 118px 80px 90px 116px 96px 84px", gap: 0, padding: "11px 16px", transition: "background.14s", background: expanded ? "rgba(255,255,255,.035)" : "transparent" }}
                    ...
                    <div style={{ fontFamily: F.sans, fontSize: 12, fontWeight: 600, color: C.white, display: "flex", alignItems: "center" }}>{c.qty.toLocaleString("it-IT")}</div>
...
const { icon, label, col } = SVC_BADGE[svc];
          ...
          {/* ULTIME NOTIFICHE */}
          <div style={{...box(), padding: "14px" }}>
            <div style={{ fontFamily: F.sans, fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,.35)", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 10 }}>Ultime attivita</div>
            {[
              ...
              { icon: "", msg: "quantita da verificare - C002 in attesa", t: "ieri" },
  ...
  return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(180deg,${C.navyDeep},${C.navyMid})`, padding: "110px 24px 80px" }}>
      <div style={{ maxWidth: 440, margin: "0 auto", background: "rgba(255,255,255,.045)", border: "1px solid rgba(255,255,255,.09)", borderRadius: 16, padding: 26, boxShadow: "0 30px 70px rgba(0,0,0,.28)" }}>
        ...
        {!configured && <div style={{ marginTop: 12, fontFamily: F.sans, fontSize: 11, color: "rgba(255,255,255,.38)", lineHeight: 1.5 }}>Modalita prototipo: la pagina e il flusso sono pronti, l'invio reale parte appena inserisci le variabili ambiente Supabase.</div>}
      ...
      <div style={{ maxWidth: 1040, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10, marginBottom: 16 }}>
          {[
            ...
            ["Volantini distribuiti", flyersDone.toLocaleString("it-IT"), C.blue],
          ].map(([l, v, c]) => (
                ...
                return (
                  <div key={campagna.id} style={{ padding: 16, borderRadius: 13, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.07)", display: "grid", gridTemplateColumns: "1fr auto", gap: 14, alignItems: "center" }}>
                    <div>
                      ...
                      <div style={{ fontFamily: F.sans, fontSize: 12, color: "rgba(255,255,255,.45)", marginTop: 5 }}>{(campagna.comuni || []).join(" – ") || "Comuni da confermare"} – {campagna.quantita.toLocaleString("it-IT")} volantini – {campagna.data_inizio || "data da definire"}  {campagna.data_fine || "fine da definire"}</div>
                    </div>
                    <div style={{ textAlign: "right" }}>
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", flexWrap: "wrap", marginBottom: 22 }}>
            <div>
              ...
              <div style={{ fontFamily: F.sans, fontSize: 13, color: "rgba(255,255,255,.45)", marginTop: 6 }}>{campagna.quantita.toLocaleString("it-IT")} volantini – Smart Pairing {campagna.smart_pairing_sconto}%</div>
            </div>
  ...
  const config = {
    ...
    terms: {
      ...
      rows: [
        ...
        ["Responsabilita", "Tempi, disponibilita e copertura possono variare in base a condizioni operative, meteo, accessibilita e conferma finale."],
      ]
    },
    cookie: {
      eyebrow: "Cookie",
      title: "Cookie Policy",
      intro: "Policy cookie minima per il prototipo. Il banner consensi e predisposto e sara collegato agli strumenti reali quando attivati.",
      rows: [
        ...
        ["Gestione consenso", "Il consenso puo essere aggiornato dal banner cookie o da questa pagina quando il modulo definitivo sara collegato."],
      ]
    }
  }[type] || {};
