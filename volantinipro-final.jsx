  ...
  useEffect(() => {
    ...
    if (prevActiveZoneIdRef.current !== data.activeZoneId) {
      ...
      setData(prev => ({
        ...
        flyerQuantity: activeZone.assigned_flyers || 10000,
        flyerQuantityFromStep1: activeZone.assigned_flyers || 10000,
        cityName: activeZone.cityName || resolvedCity?.name || "",
        city: resolvedCity,
                    ...
                    isPartial ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        <div style={{ padding: "10px 14px", borderRadius: 10, background: "rgba(232,87,26,.08)", border: "1px solid rgba(232,87,26,.28)", fontFamily: F.sans, fontSize: 11, color: C.orange, lineHeight: 1.5 }}>
                          {isResidentialStep2 ? `Copertura parziale: hai ${flyerQuantityFromStep1.toLocaleString("it-IT")} vol., servono ${requiredFlyers.toLocaleString("it-IT")}. Puoi continuare con copertura parziale.` : "Copertura parziale: quantità insufficiente. Puoi continuare con copertura parziale."}
                        </div>
          ...
          <div style={{ background: `${col}10`, borderRadius: 12, padding: "14px 16px", border: `1px solid ${col}28` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 10 }}>
              {(isResidentialStep2 ? [
                ...
                { l: "Hotspot", v: serviceKpis?.hotspotStrength ?? null, unit: "/100" },
                { l: "Inseriti", v: flyerQuantityFromStep1.toLocaleString("it-IT") },
            ...
            {selZones.length > 0 && (
              <div style={{ padding: "8px 10px", borderRadius: 8, background: coverageStatus === "sufficient" ? "rgba(46,204,138,.08)" : "rgba(248,113,113,.08)", border: `1px solid ${coverageStatus === "sufficient" ? "rgba(46,204,138,.22)" : "rgba(248,113,113,.24)"}` }}>
                <div style={{ fontFamily: F.sans, fontSize: 11, fontWeight: 700, color: coverageStatus === "sufficient" ? C.green : C.red, marginBottom: 2 }}>
                  {coverageStatus === "sufficient" ? "quantità sufficiente" : "Volantini insufficienti"}
                </div>
                <div style={{ fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.52)", lineHeight: 1.45 }}>
                  {coverageStatus === "sufficient"
                    ? `quantità sufficiente per coprire le zone selezionate. Avanzano ${flyerSurplus.toLocaleString("it-IT")} volantini.`
              ...
              {selZones.length === 0 ? (
                ...
                <div style={{ padding: "8px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                  {(isResidentialStep2 ? [
                    ...
                    { l: "Hotspot score", v: gisLoading ? gisKpi(null, 54) : serviceKpis.hotspotStrength, unit: "/100", c: C.green },
...
const remaining = campaign.qty - required;
  ...
  return {
    zones,
    rows: [
      { l: "Attività rilevate", v: zones.reduce((a, z) => a + z.bizTotal, 0).toLocaleString("it-IT") },
      ...
      { l: "Attività target", v: zones.reduce((a, z) => a + z.targetBiz, 0).toLocaleString("it-IT") },
  ...
  return (
    <div style={{ marginTop: 16, marginBottom: 48 }}>
      ...
      <p style={{ fontFamily: F.sans, fontSize: 12, lineHeight: 1.5, color: "rgba(255,255,255,.42)", maxWidth: 430, margin: "8px 0 0" }}>
        Aggiungi una seconda zona se hai più punti vendita o vuoi coprire aree separate. Potrai configurarla sulla mappa nel passaggio successivo.
      </p>
    </div>
  );
}
