    ...
    <nav style={{ position: navPosition, top: 0, left: 0, right: 0, zIndex: 200, background: dark || sc ? "rgba(5,10,20,.85)" : "transparent", borderBottom: (dark || sc) ? "1px solid rgba(255,255,255,.05)" : "none", transition: "background.35s", backdropFilter: "blur(16px)" }}>
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: isMobile ? "0 16px" : "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 72 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }} onClick={() => go("home")}>
          <div style={{ width: 28, height: 28, borderRadius: 6, background: "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="24" height="24" viewBox="0 0 32 32" fill="none"><path d="M16 3C11.03 3 7 7.03 7 12c0 7 9 17 9 17s9-10 9-17c0-4.97-4.03-9-9-9Z" fill={C.orange} /><circle cx="16" cy="12" r="3.2" fill="#ffe7dc" /></svg>
          </div>
          <span style={{ fontFamily: F.sans, fontSize: 18, fontWeight: 900, letterSpacing: "-0.03em", color: C.white }}>Volantini<span style={{ color: C.orange }}>Pro</span></span>
        </div>
        {!isMobile && <div style={{ display: "flex", gap: 42 }}>{navLinks.map(l => <a key={l} className="nl" href="#" style={{ color: "rgba(255,255,255,.65)", textDecoration: "none", fontFamily: F.sans, fontSize: 14, fontWeight: 600, transition: "color 0.2s" }}>{l}{l === "Risorse" ? <span style={{ marginLeft: 5, color: "rgba(255,255,255,.4)", fontSize: 10 }}>▾</span> : null}</a>)}</div>}
        {!isMobile && <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <button onClick={() => go("login")} style={{ padding: "10px 18px", borderRadius: 8, border: "1px solid rgba(255,255,255,.15)", background: "rgba(255,255,255,.04)", color: C.white, fontFamily: F.sans, fontSize: 14, fontWeight: 600, cursor: "pointer", transition: "all 0.2s ease" }}>Accedi</button>
          <button className="vb" onClick={() => go("step1")} style={{ padding: "10px 22px", borderRadius: 8, border: "none", background: C.orange, color: C.white, fontFamily: F.sans, fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: `0 4px 14px rgba(232, 87, 26, 0.25), inset 0 1px 0 rgba(255,255,255,0.2)` }}>Configura la tua campagna</button>
        </div>}
...
const operationalSummary = svcType === "d2d" ? (quantityIsSufficient
    ? `La campagna copre ${mainAreaLabel} con una stima di ${estimatedFamiliesForSummary.toLocaleString("it-IT")} famiglie e ${coverageForSummary}% di copertura. La quantità inserita è sufficiente; restano ${remainingQty.toLocaleString("it-IT")} volantini disponibili per estensione zona o scorta operativa.`
    : `La quantità inserita non copre completamente l'area selezionata. Mancano ${missingQty.toLocaleString("it-IT")} volantini per raggiungere la copertura stimata.`)
    : svcType === "h2h"
          ...
          <div style={{...box(), padding: "18px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(155px,1fr))", gap: 8 }}>
              {nonEmpty([
                ...
                { icon: "", l: "quantità", v: flyerQty.toLocaleString("it-IT") + " pz.", c: C.white },
                ...
                (data.subscription && data.subscription !== "single") && { icon: "", l: "quantità/campagna", v: flyerQty.toLocaleString("it-IT") + " pz.", c: C.white },
...
const remaining = campaign.qty - required;
    return {
      ...
      rows: [
        ...
        { l: remaining >= 0 ? "quantità residua" : "quantità mancante", v: Math.abs(remaining).toLocaleString("it-IT") },
