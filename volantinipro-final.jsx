...
function Step1PeriodPresets({value,onChange,isMobile}){const opts=[{id:"asap",label:"Appena possibile"},{id:"within7",label:"Entro 7 giorni"},{id:"within15",label:"Entro 15 giorni"},{id:"custom",label:"Scelgo una data"}];return _jsx("div",{style:{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:8,marginBottom:12},children:opts.map(opt=>{const active=(value||"custom")===opt.id;return _jsx("button",{type:"button",onClick:()=>onChange(opt.id),style:{padding:"9px 10px",borderRadius:9,border:`1.5px solid ${active?C.orange:"rgba(255,255,255,.1)"}`,background:active?"rgba(232,87,26,.12)":"rgba(255,255,255,.04)",color:active?C.orange:"rgba(255,255,255,.62)",fontFamily:F.sans,fontSize:11,fontWeight:800,cursor:"pointer"},children:opt.label},opt.id)})})}
function Step1ConfigSummary({data}){
  ...
  const print=data.hasFlyers==="yes"?"Gia stampati":data.hasFlyers==="no"?"Da stampare":"Da selezionare";
  const fmt=data.flyerFormat?String(data.flyerFormat).toUpperCase():"Da selezionare";
  const qty=data.qty?Number(data.qty).toLocaleString("it-IT"):"Da selezionare";
  const basePrint=data.hasFlyers==="no"&&data.qty?Math.round(Number(data.qty||0)/1000*29):0;
  const partialSubtotal=basePrint;
  const rows=[["Servizio",service],["Quantita",qty],["Formato",fmt],["Urgenza",urgency],["Piano",plan],["Stampa",print]];
  return _jsxs("div",{style:{position:"sticky",bottom:18,zIndex:5,marginBottom:18,padding:"14px",borderRadius:12,background:"rgba(255,255,255,.045)",border:"1px solid rgba(255,255,255,.1)",backdropFilter:"blur(10px)",boxShadow:"0 18px 42px rgba(0,0,0,.18)"},children:[
    ...
    _jsx("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:8},children:rows.map(([label,val])=>_jsxs("div",{style:{padding:"9px 10px",borderRadius:9,background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.055)"},children:[_jsx("div",{style:{fontFamily:F.sans,fontSize:9,color:"rgba(255,255,255,.36)",textTransform:"uppercase",letterSpacing:".06em",marginBottom:3},children:label}),_jsx("div",{style:{fontFamily:F.sans,fontSize:12,fontWeight:800,color:val==="Da selezionare"?"rgba(255,255,255,.34)":C.white},children:val})]},label))}),
    _jsxs("div",{style:{display:"grid",gap:7,marginTop:10,padding:"11px 12px",borderRadius:10,background:"rgba(232,87,26,.07)",border:"1px solid rgba(232,87,26,.18)"},children:[
      _jsxs("div",{style:{display:"flex",justifyContent:"space-between",gap:12,fontFamily:F.sans,fontSize:12,color:"rgba(255,255,255,.72)"},children:[_jsx("span",{children:"Distribuzione"}),_jsx("b",{style:{color:C.white},children:"Calcolata sulla tua zona (Step 2)"})]}),
      basePrint>0&&_jsxs("div",{style:{display:"flex",justifyContent:"space-between",gap:12,fontFamily:F.sans,fontSize:12,color:"rgba(255,255,255,.72)"},children:[_jsx("span",{children:"Stampa indicativa"}),_jsxs("b",{style:{color:C.orange},children:["EUR ",basePrint.toLocaleString("it-IT")]})]}),
      _jsxs("div",{style:{display:"flex",justifyContent:"space-between",gap:12,fontFamily:F.sans,fontSize:13,color:C.white,borderTop:"1px solid rgba(255,255,255,.08)",paddingTop:7},children:[_jsx("span",{children:"Subtotale parziale"}),_jsxs("b",{style:{color:C.orange},children:["EUR ",partialSubtotal.toLocaleString("it-IT")]})]})
    ]}),
    _jsxs("p",{style:{margin:"10px 0 0",fontFamily:F.sans,fontSize:11,lineHeight:1.45,color:"rgba(255,255,255,.45)"},children:["Configurazione attuale: distribuzione da calcolare sulla zona · piano ",plan," · formato ",fmt]})
  ]})
}
                ...
                {shouldGroupMarginalZones && (
                  <div style={{ padding: "10px 12px", borderRadius: 10, background: "rgba(232,87,26,.08)", border: "1px solid rgba(232,87,26,.22)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                    <div style={{ fontFamily: F.sans, fontSize: 11, color: "rgba(255,255,255,.62)", lineHeight: 1.45 }}>
                      ...
                      {primaryCoveredZones.length > 0 && <> Con {flyerQuantityFromStep1.toLocaleString("it-IT")} volantini coprirai principalmente: <b style={{ color: col }}>{primaryCoveredZones.join(", ")}</b>.</>}
                    </div>
                    ...
                    return (
                      <div key="marginal-summary" style={{ borderRadius: 10, border: "1px dashed rgba(255,255,255,.13)", background: "rgba(255,255,255,.025)", padding: "9px 10px" }}>
                        ...
                        <div style={{ marginTop: 5, fontFamily: F.sans, fontSize: 10, color: "rgba(255,255,255,.42)" }}>
                          totale aggregato: <b style={{ color: C.white }}>{marginalZoneFamilies.toLocaleString("it-IT")}</b> famiglie · <b style={{ color: C.white }}>{marginalZoneCoverage}%</b> del raggio
                        </div>
                      </div>
                    );
                  }
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 8 }}>
                          <button onClick={handleNext}
                            ...
                            Procedi con i miei {flyerQuantityFromStep1.toLocaleString("it-IT")} volantini
                          </button>
                          <button onClick={() => setData(d => ({...d, qty: requiredFlyers, flyerQuantity: requiredFlyers }))}
                            ...
                            Aumenta a {requiredFlyers.toLocaleString("it-IT")} vol. (copertura completa)
                          </button>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          ...
          {city && isMovementStep2 && (
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, border: `1px solid ${col}22`, overflow: "hidden" }}>
              <div style={{ padding: "9px 14px", borderBottom: "1px solid rgba(255,255,255,.06)", display: "flex", alignItems: "center", gap: 7 }}>
                <div style={{ width: 16, height: 3, background: col, borderRadius: 2 }} />
                <span style={{ fontFamily: F.sans, fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,.32)", letterSpacing: ".1em", textTransform: "uppercase", flex: 1 }}>MobilitÃƒ  e attrattori H2H</span>
              ...
              <div style={{ padding: "8px 10px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                {[
                  ...
                  { l: "UniversitÃƒ ", v: serviceKpis.universities, c: C.yellow },
