...
export const F=A.split("-");return F.length===3?`${F[2]}/${F[1]}/${F[0]}`:""},T=A=>{const F=A.replace(/\D/g,"").slice(0,8);return F.length<=2?F:F.length<=4?`${F.slice(0,2)}/${F.slice(2)}`:`${F.slice(0,2)}/${F.slice(2,4)}/${F.slice(4)}`},z=A=>{const F=A.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);if(!F)return"";
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          ...
          {city && isMovementStep2 && (
            <div style={{ background: "rgba(255,255,255,.04)", borderRadius: 12, border: `1px solid ${col}22`, overflow: "hidden" }}>
              <div style={{ padding: "9px 14px", borderBottom: "1px solid rgba(255,255,255,.06)", display: "flex", alignItems: "center", gap: 7 }}>
                <div style={{ width: 16, height: 3, background: col, borderRadius: 2 }} />
                <span style={{ fontFamily: F.sans, fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,.32)", letterSpacing: ".1em", textTransform: "uppercase", flex: 1 }}>MobilitÃ  e attrattori H2H</span>
              ...
              <div style={{ padding: "8px 10px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                {[
                  ...
                  { l: "UniversitÃ ", v: serviceKpis.universities, c: C.yellow },
