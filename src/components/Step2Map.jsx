...
function dotIcon(L, color, size, selected) {
  const s = size || (selected ? 10 : 6);
  const shadow = `0 1px 4px rgba(0,0,0,0.40)`;
...
function LayerPanel({ config, activeLayers, settori, onToggle }) {
      ...
      {!collapsed && (
        <div style={{ padding: '4px 0 3px' }}>
          {config.map(layer => {
            ...
            const isOn = activeLayers?.[layer.id] ?? layer.defaultOn ?? false;
            const active = isOn && !layer.future;
            ...
            return (
              <div
                key={layer.id}
                onClick={!layer.future ? () => onToggle?.(layer.id) : undefined}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: 0,
                  cursor: layer.future ? 'default' : 'pointer',
                  opacity: layer.future ? 0.32 : 1,
                  transition: 'background 0.12s',
                  position: 'relative',
                }}
                onMouseEnter={e => { if (!layer.future) e.currentTarget.style.background = 'rgba(255,255,255,0.04)'; }}
              ...
              >
                ...
                {!layer.future && !settoriNoData && (
                  <div style={{
                    width: 6, height: 6, borderRadius: '50%', marginRight: 10, flexShrink: 0,
...
export function Step2Map({
  ...
  useEffect(() => {
    ...
    const col = serviceColor || '#C4852A';
    map.setView([city.lat, city.lng], getZoomForRadius(radius));
    ...
    zonesList.forEach(z => {
      ...
      const zRadius = parseFloat(z.radius ?? z.radius_km ?? 3);
      const zSvc = z.service_type || 'd2d';
      const zCol = isActive ? col : '#7F9BB0'; // Soft desaturated color for inactive zones
      ...
      if (isRingOn) {
        L.circle([zCity.lat, zCity.lng], {
          ...
          color: zCol,
          fillColor: zCol,
          fillOpacity: isActive ? 0.04 : 0.015,
          weight: isActive ? 1.8 : 1.1,
          dashArray: isActive ? '10 7' : '5 5',
          opacity: isActive ? 0.60 : 0.35,
    ...
    if (activeLayers?.comuni !== false && zonesWithCoords?.length > 0) {
      zonesWithCoords.forEach(z => {
        ...
        const styleUnsel = {
          color:       settoriActive ? '#5A7A9A'     : ZONE_UNSEL.color,
          fillColor:   settoriActive ? '#6A8AAA'     : ZONE_UNSEL.fill,
          fillOpacity: settoriActive ? 0.03          : 0.12,
          weight:      settoriActive ? 0.7           : 1.5,
          opacity:     settoriActive ? 0.20          : 0.78,
          dashArray:   '5 4',
        };
        const styleSel = {
          color: col, fillColor: col,
          fillOpacity: settoriActive ? 0.13 : 0.20,
          weight: settoriActive ? 2.2 : 2,
          opacity: settoriActive ? 0.82 : 0.88,
    ...
    if (settoriActive) {
      ...
      settori.forEach((s, idx) => {
        ...
        try {
          ...
          // Hover effect
          poly.on('mouseover', () => poly.setStyle({ fillOpacity: isSel ? 0.30 : 0.22, weight: isSel ? 2.5 : 2.2 }));
    ...
    if (poiActive && pois?.length > 0) {
      ...
      clusters.forEach(item => {
        if (item.isCluster) {
          ...
          const s = Math.min(26, 14 + Math.log2(item.count) * 3);
          const clusterIcon = L.divIcon({
            html: `<div style="
              width:${s}px;height:${s}px;border-radius:50%;
              background:${item.color};opacity:0.92;
          ...
          const sz = item.priority >= 9 ? 10 : item.priority >= 7 ? 8 : 6;
          const isSel = item.priority >= 8;
  ...
  }, [leafletLoaded, city, radius, zonesWithCoords, selected, apiData, svcType, serviceColor, targetColor, activeLayers, settori, selectedSectorId, pois, mapZoom]); // eslint-disable-line react-hooks/exhaustive-deps
