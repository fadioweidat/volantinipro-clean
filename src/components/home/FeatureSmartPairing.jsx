...
const statuses = [
  ...
  { label: "Smart Pairing Zona Vicina", value: "Risparmio -20%", active: true, color: "#A78BFA" },
  ...
  return (
    <section className="section" style={{ background: "#111827", paddingTop: 120, paddingBottom: 120, paddingLeft: 28, paddingRight: 28, boxSizing: "border-box", borderTop: "1px solid rgba(148,163,184,0.18)" }}>
      <div className="smart-pairing-layout" style={{ maxWidth: 1160, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(320px, 100%), 1fr))", gap: 80, alignItems: "center" }}>
        <div>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "5px 14px", borderRadius: 100, background: "linear-gradient(135deg, rgba(99,102,241,0.15), rgba(6,182,212,0.15))", border: "1px solid rgba(6,182,212,0.3)", marginBottom: 18 }}>
            <span style={{ fontSize: 13 }}>✨</span>
            <span style={{ fontFamily: F.sans, fontSize: 11, fontWeight: 800, color: C_CYAN, textTransform: "uppercase", letterSpacing: ".08em" }}>Smart Pairing AI</span>
          </div>
          ...
          <div className="vc" style={{ borderRadius: 24, overflow: "hidden", background: "rgba(24,34,53,0.6)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)", border: "1px solid rgba(148,163,184,0.2)", boxShadow: "0 32px 64px rgba(0,0,0,.35)" }}>
            ...
            <div style={{ padding: 24, display: "grid", gap: 12 }}>
              {statuses.map((status) => (
                <div key={status.label} style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", padding: "16px 20px", borderRadius: 14, background: status.active ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.02)", border: `1px solid ${status.active ? "rgba(255,255,255,0.12)" : "rgba(148,163,184,0.08)"}` }}>
            ...
            <div style={{ margin: "0 24px 24px", padding: "16px 20px", borderRadius: 14, background: "rgba(99,102,241,0.08)", border: "1px solid rgba(99,102,241,0.2)", fontFamily: F.sans, fontSize: 12, color: "#CBD5E1", lineHeight: 1.6 }}>
        ...
        <div>
          ...
          <h2 style={{ fontFamily: F.serif, fontSize: "clamp(36px, 3vw, 52px)", lineHeight: 1.05, color: "#F8FAFC", letterSpacing: "-0.03em", margin: "0 0 22px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            {features.map((f, i) => (
              <div key={f.label} style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                <span style={{ width: 28, height: 28, borderRadius: 10, background: "rgba(99,102,241,0.15)", border: "1px solid rgba(99,102,241,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: F.sans, fontSize: 11, fontWeight: 900, color: "#818CF8", flexShrink: 0, marginTop: 2 }}>{String(i + 1).padStart(2, "0")}</span>
                <div>
          ...
          <Button variant="primary" className="vb" onClick={onConfigure} style={{ marginTop: 42, minHeight: 48, padding: "0 32px", borderRadius: 12, fontFamily: F.sans, fontSize: 14, fontWeight: 900, boxShadow: "0 12px 32px rgba(99,102,241,0.25)" }}>
            Configura la tua campagna con AI
          </Button>
        </div>
      </div>
    </section>
