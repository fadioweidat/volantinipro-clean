...
export default function FeatureSmartPairing({ onConfigure }) {
  return (
    <section style={{ background: "#FAF9F7", padding: "96px 28px", boxSizing: "border-box" }}>
      <div style={{ maxWidth: 1160, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 72, alignItems: "center" }}>
        ...
        <div>
          <div style={{ fontFamily: F.sans, fontSize: 11, fontWeight: 800, letterSpacing: ".14em", textTransform: "uppercase", color: CO, marginBottom: 18 }}>
            Step 03 — Pianificazione
          </div>
          <h2 style={{ fontFamily: F.serif, fontSize: "clamp(36px, 3vw, 54px)", lineHeight: 1.08, color: "#1A1A1A", letterSpacing: "-0.03em", margin: "0 0 22px" }}>
            Smart Pairing:<br />risparmia distribuendo insieme
          </h2>
          <p style={{ fontFamily: F.sans, fontSize: 17, lineHeight: 1.7, color: "rgba(26,26,26,.6)", margin: "0 0 38px", maxWidth: 440 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {features.map((f, i) => (
              <div key={f.label} style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                ...
                <div>
                  <div style={{ fontFamily: F.sans, fontSize: 14, fontWeight: 800, color: "#1A1A1A", marginBottom: 3 }}>{f.label}</div>
                  <div style={{ fontFamily: F.sans, fontSize: 13, lineHeight: 1.55, color: "rgba(26,26,26,.52)" }}>{f.desc}</div>
                </div>
              </div>
            ))}
          </div>
