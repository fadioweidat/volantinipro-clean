...
export function Step2({
  ...
  const {
    ...
    loading: apiLoading,
    error: apiError
  } = useServiceAnalysis(analysisParams.lat, analysisParams.lng, analysisParams.radiusKm, analysisParams.serviceType, analysisParams.municipality, analysisParams.quantity, analysisParams.scope, analysisParams.analysisLevel, analysisParams.selectionScope, analysisParams.selectedMunicipalityCodes);
              ...
              return <div id="vp-step2-zone-details-panel" className="vp-step2-zone-details__panel">
                    ...
                    <table className="vp-step2-zone-table">
                      ...
                      return <React.Fragment key={rowKey}>
                            <tr>
                              <th scope="row">
                                <button type="button" onClick={() => toggleZoneRowDetail(rowKey)} aria-expanded={isRowExpanded} aria-controls={`vp-zone-detail-${rowKey}`} style={{
                              display: "flex",
                              alignItems: "center",
                              gap: 7,
                              background: "none",
                            ...
                            {isRowExpanded && <tr id={`vp-zone-detail-${rowKey}`}>
                                <td colSpan={6} style={{
                            ...
                            padding: "12px 14px"
                              ...
                              margin: 0
                                    ...
                                    {[["Comune", comuneLabel], ["Famiglie/cassette stimate", target > 0 ? formatIntegerIT(target) : "Dato non disponibile"], ["Quantità assegnata", formatIntegerIT(assigned)], ["Copertura", coverage != null ? formatPercentIT(coverage, Number.isInteger(coverage) ? 0 : 1) : "Dato non disponibile"], ["Priorità", priorityLabel], ["Stato", status]].map(([dt, dd]) => <div key={dt}>
                                        <dt style={{
                                  fontFamily: F.sans,
                                  fontSize: 9,
                                  fontWeight: 800,
