...
export function Step2MapPanel({ simplifiedMilano = false, containingNil, activeLay, activeMapLayers, apiData, boundaryKpisForMap, city, civiciAvailable, civiciState, col, data, dusafLanduse, focusedPoiId, focusedPoiNonce, focusedNil, gisLoading, gisSlowConnection, gisTimedOut, onRetryGis, handleManualMapClick, hasUnconfirmedAddressPoint, hiddenBoundaries, isAdminView, isBusinessStep2, isComuneMode, isMovementStep2, isNilAnalysis, isNilManualMode, isRadiusMode, manualPinMode, mapBasemap, mapCityForStep2, mapConfiniOn, mapCoverageZones, municipalityBoundary, omiInfo, pois, poiEmptySectorLabel, poiFetchFailed, poiLoading, poiAssignments, radius, radiusKm, residentialRadiusRows, retryPoi, sectors, searchMode, selected, selectedSearchPoint, selectCampaignZone, selZones, serviceKpis, setActiveMapLayers, setManualPinMode, setMapBasemap, setMapConfiniOn, sharedCoveragePctText, showTerritoryData, step1OperationalPoints, svcType, targetBusinessMeta, thMax, thMin, toggleZone, togglePoiAssignment, viewMode, zoneAllocationById, zoneCoverageById, zonesInRadius, zonesWithCoords }) {
  return (
    <>
      {simplifiedMilano && <div className="vp-milano-map-toolbar" role="group" aria-label="Layer della mappa">
        <span>Mostra sulla mappa</span>
        {isRadiusMode && <button type="button" data-testid="map-toggle-comuni" aria-pressed={activeMapLayers?.comuni !== false} onClick={() => setActiveMapLayers(prev => ({ ...prev, comuni: prev.comuni === false }))}>Comuni {activeMapLayers?.comuni !== false ? "ON" : "OFF"}</button>}
        <button type="button" data-testid="map-toggle-nil" aria-pressed={activeMapLayers?.nil !== false} onClick={() => setActiveMapLayers(prev => ({ ...prev, nil: prev.nil === false }))}>NIL / Quartieri {activeMapLayers?.nil !== false ? "ON" : "OFF"}</button>
        <small>{isNilManualMode ? "Clicca un quartiere per aggiungerlo o rimuoverlo." : "I confini sono un riferimento: non selezionano una zona."}</small>
      </div>}
      {/* MAPPA GRANDE — solo Vista Cliente. */}
          <div style={{
            ...
            <Step2Map key={simplifiedMilano ? "milano-client" : "standard"} simplifiedMilano={simplifiedMilano} containingNil={containingNil} city={mapCityForStep2} radius={isRadiusMode ? Number(radiusKm) || Number(radius) || 3 : radiusKm} svcType={svcType} serviceColor={col} zonesWithCoords={zonesWithCoords} selected={selected} onToggleZone={toggleZone} apiData={apiData} targetColor={targetBusinessMeta?.color || '#a78bfa'} activeLayers={activeMapLayers} settori={sectors} pois={pois} loadingPois={poiLoading} poiEmptySectorLabel={poiEmptySectorLabel} poiFetchFailed={poiFetchFailed} onRetryPoi={retryPoi} operationalPoints={step1OperationalPoints} poiAssignments={poiAssignments} onTogglePoi={togglePoiAssignment} focusPoiId={focusedPoiId} focusPoiNonce={focusedPoiNonce} focusNil={focusedNil}businessConfig={isBusinessStep2 ? {
