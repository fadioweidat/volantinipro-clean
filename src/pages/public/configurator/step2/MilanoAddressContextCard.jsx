...
export default function MilanoAddressContextCard({ addressPoint, coverageAddress, containingNil, onUseNil, onPreviewMunicipio, onUseRadius, onKeepMilanoComplete }) {
  const [derived, setDerived] = useState({ municipio: null, cap: null, loading: false });
  ...
  useEffect(() => {
    if (!addressPoint || !Number.isFinite(Number(addressPoint.lat)) || !Number.isFinite(Number(addressPoint.lng))) {
      setDerived({ municipio: null, cap: null, loading: false });
      return undefined;
    }
    const controller = new AbortController();
    let current = true;
    setDerived({ municipio: null, cap: null, loading: true });
    Promise.allSettled([
    ...
    ]).then(([municipioResult, capResult]) => {
      if (!current) return;
      setDerived({ municipio: municipioResult.status === 'fulfilled' ? municipioResult.value : null, cap: capResult.status === 'fulfilled' ? capResult.value : null, loading: false });
    });
    return () => { current = false; controller.abort(); };
  }, [identity]);

  const context = useMemo(() => buildMilanoAddressContext({ addressPoint, coverageAddress, nil: containingNil, municipio: derived.municipio, cap: derived.cap }), [addressPoint, coverageAddress, containingNil, derived]);
  if (!addressPoint) return null;
  const values = [
    ['Comune', `${context.comune.name} (${context.comune.province})`], ['NIL / quartiere', context.nil.name || 'NIL non rilevato'],
    ['Municipio', context.municipio.name || (derived.loading ? 'Rilevamento…' : 'Municipio non rilevato')],
    ['CAP', context.cap.code || (derived.loading ? 'Rilevamento…' : 'Non disponibile')],
  ...
  return (
    <section data-testid="milano-address-context" aria-label="Contesto territoriale dell'indirizzo" style={{ background: 'rgba(232,87,26,.07)', border: '1px solid rgba(232,87,26,.3)', borderRadius: 12, padding: '13px 14px', display: 'flex', flexDirection: 'column', gap: 10, fontFamily: F.sans }}>
      ...
      <p style={{ margin: 0, color: 'rgba(255,255,255,.46)', fontSize: 9.5, lineHeight: 1.4 }}>Il CAP è informativo e non modifica area, copertura, quantità o prezzo.</p>
    </section>
  );
}
